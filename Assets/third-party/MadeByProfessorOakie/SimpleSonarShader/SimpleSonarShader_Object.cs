...
public class SimpleSonarShader_Object : MonoBehaviour
{
    ...
    private static readonly int QueueSize = 20;
    ...
    private static readonly Queue<Vector4> positionsQueue = new Queue<Vector4>(QueueSize);

    // Queue of intensity values for each ring.
    // These are kept in the same order as the positionsQueue.
    private static readonly Queue<float> intensityQueue = new Queue<float>(QueueSize);
