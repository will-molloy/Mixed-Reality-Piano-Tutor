...
public class BallLauncher : MonoBehaviour
{
    ...
    /// </summary>
    private readonly Vector3 offset = new Vector3(0.1f, -0.1f, 0.0f);
    ...
    /// </summary>
    private readonly float timeBallMax = 0.05f;
    ...
    private void Start()
    {
        ...
        for (var i = 0; i < 10; i++) ballColors[i] = Color.HSVToRGB(0.1f * i, 0.8f, 1.0f);
        ...
        for (var i = 0; i < SPHERE_NB; i++)
        {
            ...
            var l = projectiles[i].AddComponent<Light>();
    ...
    private void Update()
    {
        if (ready && (Input.GetKey(KeyCode.Space) || Input.GetButton("Fire1")))
        {
            if (timerBall > timeBallMax)
            {
                ...
                var offsetAngleX = 0.0f;
                var offsetAngleY = 0.0f;


                launcher.transform.localRotation =
                    Quaternion.Euler(-offsetAngleY * Mathf.Rad2Deg, -offsetAngleX * Mathf.Rad2Deg, 0);
                projectiles[countSphere % SPHERE_NB].GetComponent<BallTrigger>().ResetValues();
                var rigidBody = projectiles[countSphere % SPHERE_NB].GetComponent<Rigidbody>();
        ...
        for (var i = 0; i < SPHERE_NB; i++)
            ...
            {
                if (Vector3.Distance(projectiles[i].transform.position, Vector3.zero) > DISTANCE_MAX ||
                    times[i] > TIME_MAX)
                {
                    projectiles[i].SetActive(false);
                    times[i] = 0;
