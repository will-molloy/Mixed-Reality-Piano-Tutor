...
public class NavMeshAgentController : MonoBehaviour
{
    ...
    private bool RandomPoint(Vector3 center, float range, out Vector3 result)
    {
        for (var i = 0; i < 30; i++)
        {
            var vector = Random.insideUnitSphere * range;
            if (Vector3.Dot(vector, -Vector3.up) < 0) vector = -vector;
            RaycastHit rayHit;
            if (!Physics.Raycast(center + transform.up, vector, out rayHit)) continue;
            var randomPoint = rayHit.point;
