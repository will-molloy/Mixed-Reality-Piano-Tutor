...
namespace sl
{
    ...
    public class ZEDCamera
    {
        ...
        private const int tagZEDCamera = 20;

	    /// <summary>
	    ///     Layer only the other cameras apart the ZED are able to see
	    /// </summary>
	    private const int tagOneObject = 12;

        //Singleton of ZEDCamera
        private static ZEDCamera instance;
	    ...
	    /// </summary>
	    private static readonly string[] dependenciesNeeded =
        {
            "sl_zed64.dll",
            "sl_core64.dll",
            "sl_input64.dll"
	    ...
	    /// </summary>
	    public static readonly Version PluginVersion = new Version(2, 5, 0);
        ...
        private readonly ZEDCameraSettingsManager cameraSettingsManager = new ZEDCameraSettingsManager();
        ...
        private readonly Dictionary<int, Dictionary<int, Texture2D>> textures;

        //List of all requested textures
        private readonly List<TextureRequested> texturesRequested;
	    ...
	    ///     Current field of view
	    /// </summary>
	    public float HorizontalFieldOfView => fov_H;
	    ...
	    /// </summary>
	    public CalibrationParameters CalibrationParametersRaw => calibrationParametersRaw;

	    /// <summary>
	    ///     Information in cache, call GetInformation to update
	    /// </summary>
	    public CalibrationParameters CalibrationParametersRectified { get; private set; }

	    /// <summary>
	    ///     Information in cache, call GetInformation to update
	    /// </summary>
	    public MODEL CameraModel => cameraModel;

        public bool IsCameraReady => cameraReady;


        public bool IsHmdCompatible => cameraModel == MODEL.ZED_M;

	    /// <summary>
	    ///     Layer only the ZED is able to see
	    /// </summary>
	    public static int Tag => tagZEDCamera;

        public static int TagOneObject => tagOneObject;
        ...
        private static extern IntPtr dllz_create_camera(bool verbose);
        ...
        */
        ...
        private static extern int dllz_open(ref dll_initParameters parameters, StringBuilder svoPath,
        ...
        private static extern int dllz_grab(ref RuntimeParameters runtimeParameters);
        ...
        */
        ...
        private static extern void dllz_get_camera_imu_transform(ulong timeStamp, bool useLatency,
        ...
        private static extern int dllz_enable_tracking(ref Quaternion quat, ref Vector3 vec,
            bool enableSpatialMemory = false, bool enablePoseSmoothing = false, StringBuilder aeraFilePath = null);

        [DllImport(nameDll, EntryPoint = "dllz_disable_tracking")]
        private static extern void dllz_disable_tracking(StringBuilder path);

        [DllImport(nameDll, EntryPoint = "dllz_save_current_area")]
        private static extern int dllz_save_current_area(StringBuilder path);
        ...
        private static extern int dllz_get_position_at_target_frame(ref Quaternion quaternion, ref Vector3 translation,
            ref Quaternion targetQuaternion, ref Vector3 targetTranslation, int reference_frame);
        ...
        private static extern int dllz_reset_tracking_with_offset(Quaternion rotation, Vector3 translation,
            Quaternion offsetQuaternion, Vector3 offsetTranslation);
        ...
        */
        ...
        private static extern int dllz_enable_spatial_mapping(float resolution_meter, float max_range_meter,
            int saveTexture);
        ...
        private static extern int dllz_update_mesh(int[] nbVerticesInSubemeshes, int[] nbTrianglesInSubemeshes,
        ...
        private static extern int dllz_retrieve_mesh(Vector3[] vertices, int[] triangles, int nbSubmesh, Vector2[] uvs,
            IntPtr textures);
        ...
        private static extern bool dllz_load_mesh(string filename, int[] nbVerticesInSubemeshes,
        ...
        private static extern bool dllz_apply_texture(int[] nbVerticesInSubemeshes, int[] nbTrianglesInSubemeshes,
            ref int nbSubmeshes, int[] updatedIndices, ref int nbVertices, ref int nbTriangles, int[] textureSize,
            int nbSubmesh);

        [DllImport(nameDll, EntryPoint = "dllz_filter_mesh")]
        private static extern bool dllz_filter_mesh(FILTER meshFilter, int[] nbVerticesInSubemeshes,
            int[] nbTrianglesInSubemeshes, ref int nbSubmeshes, int[] updatedIndices, ref int nbVertices,
        ...
        private static extern void dllz_spatial_mapping_merge_chunks(int numberFaces, int[] nbVerticesInSubemeshes,
            int[] nbTrianglesInSubemeshes, ref int nbSubmeshes, int[] updatedIndices, ref int nbVertices,
            ref int nbTriangles, int nbSubmesh);
        ...
        private static extern IntPtr dllz_find_floor_plane(out Quaternion rotation, out Vector3 translation,
            Vector3 priorQuaternion, Vector3 priorTranslation);
        ...
        private static extern int dllz_convert_floorplane_to_mesh(Vector3[] vertices, int[] triangles,
            out int numVertices, out int numTriangles);

        [DllImport(nameDll, EntryPoint = "dllz_convert_hitplane_to_mesh")]
        private static extern int dllz_convert_hitplane_to_mesh(Vector3[] vertices, int[] triangles,
            out int numVertices, out int numTriangles);
        ...
        private static extern IntPtr dllz_compute_optical_center_offsets(ref Vector4 calibLeft, ref Vector4 calibRight,
            Resolution imageResolution, float planeDistance);
        ...
        private static extern int dllz_retrieve_measure(IntPtr ptr, int type, int mem, Resolution resolution);

        [DllImport(nameDll, EntryPoint = "dllz_retrieve_image")]
        private static extern int dllz_retrieve_image(IntPtr ptr, int type, int mem, Resolution resolution);
	    ...
	    private static uint GetFpsForResolution(RESOLUTION reso)
        {
            ...
            if (reso == RESOLUTION.HD2K) return 15;
            if (reso == RESOLUTION.HD720) return 60;
            if (reso == RESOLUTION.VGA) return 100;
            return 30;
        }
        ...
        {
            pluginIsReady = false;
            var env = Environment.GetEnvironmentVariable("ZED_SDK_ROOT_DIR");
            if (env != null)
            {
                var error = CheckDependencies(Directory.GetFiles(env + "\\bin"));
                if (error)
                {
	    ...
	    /// <returns></returns>
	    private static bool CheckDependencies(string[] filesFound)
        {
            var isASDKPb = false;
            if (filesFound == null) return true;
            foreach (var dependency in dependenciesNeeded)
            {
                var found = false;
                foreach (var file in filesFound)
                    if (Path.GetFileName(file).Equals(dependency))
                    {
                        found = true;
                        break;
                    }
	    ...
	    public static ZEDCamera GetInstance()
        {
            lock (_lock)
            {
                if (instance == null)
                {
                    ...
                    if (CheckPlugin())
                        dllz_register_callback_debuger(DebugMethod);
                }
        ...
        {
            var infoSystem = SystemInfo.graphicsDeviceType.ToString().ToUpper();
	    ...
	    /// <summary>
	    ///     The Init function must be called after the instantiation. The function checks if the ZED camera is plugged and
	    ///     opens it, initialize the projection matix and command buffers to update textures
	    /// </summary>
	    ...
	    /// <param name="self_calib">
	    ///     if set to true, it will disable self-calibration and take the initial calibration parameters
	    ///     without optimizing them
	    /// </param>
	    /// <returns>
	    ///     ERROR_CODE : The error code gives information about the
	    ...
	    public ERROR_CODE Init(ref InitParameters initParameters)
        {
            ...
            if (initParameters.cameraFPS == 0) initParameters.cameraFPS = (int) fpsMax;

            var initP = new dll_initParameters(initParameters);
            initP.coordinateSystem = COORDINATE_SYSTEM.LEFT_HANDED_Y_UP;

            initParameters.sdkVerboseLogFile = "D:/tmp/sdl_log.txt";
            var v = dllz_open(ref initP, new StringBuilder(initParameters.pathSVO, initParameters.pathSVO.Length),
                new StringBuilder(initParameters.sdkVerboseLogFile, initParameters.sdkVerboseLogFile.Length));
            ...
            VerticalFieldOfView = CalibrationParametersRectified.leftCam.vFOV * Mathf.Deg2Rad;
	    ...
	    /// <summary>
	    ///     Fill the projection matrix with the parameters of the ZED, needs to be called only once. This projection matrix is
	    ///     off center.
	    /// </summary>
        ...
        {
            var parameters = GetCalibrationParameters(false);
            var fovx = parameters.leftCam.hFOV * Mathf.Deg2Rad;
            var fovy = parameters.leftCam.vFOV * Mathf.Deg2Rad;

            var f_imageWidth = (float) ImageWidth;
            var f_imageHeight = (float) ImageHeight;
	    ...
	    ///     true otherwise.
	    /// </returns>
	    public ERROR_CODE Grab(ref RuntimeParameters runtimeParameters)
        {
            return (ERROR_CODE) dllz_grab(ref runtimeParameters);
        }
	    ...
	    /// </summary>
	    /// <returns>
	    ///     ERRCODE : error boolean value : the function returns false if no problem was encountered,
	    ///     true otherwise.
	    ...
	    public ERROR_CODE EnableRecording(string videoFileName,
            SVO_COMPRESSION_MODE compressionMode = SVO_COMPRESSION_MODE.LOSSLESS_BASED)
        {
        ...
        {
            if (GetFpsForResolution(currentResolution) >= fps) fpsMax = (uint) fps;
	    ...
	    /// </summary>
	    /// <returns>
	    ///     Current time stamp in ns. -1 is not available(SVO file without compression).
	    ///     Note that new SVO file from SDK 1.0.0 (with compression) contains the camera time stamp for each frame.
	    /// </returns>
	    ...
	    /// <summary>
	    ///     Get the current time stamp at the time the function is called. Can be compared to the camera time stamp for
	    ///     synchronization.
	    ///     Use this function to compare the current time stamp and the camera time stamp, since they have the same reference
	    ///     (Computer start time).
	    /// </summary>
	    ...
	    /// <returns></returns>
	    public ERROR_CODE EnableTracking(ref Quaternion quat, ref Vector3 vec, bool enableSpatialMemory = true,
            bool enablePoseSmoothing = false, string areaFilePath = "")
        {
            var trackingStatus = ERROR_CODE.CAMERA_NOT_DETECTED;
            trackingStatus = (ERROR_CODE) dllz_enable_tracking(ref quat, ref vec, enableSpatialMemory,
                enablePoseSmoothing, new StringBuilder(areaFilePath, areaFilePath.Length));
	    ...
	    /// <param name="translation"></param>
	    /// <returns></returns>
	    public ERROR_CODE ResetTracking(Quaternion rotation, Vector3 translation)
        {
            var trackingStatus = ERROR_CODE.CAMERA_NOT_DETECTED;
            trackingStatus = (ERROR_CODE) dllz_reset_tracking(rotation, translation);
            return trackingStatus;
        }

        public ERROR_CODE ResetTrackingWithOffset(Quaternion rotation, Vector3 translation, Quaternion rotationOffset,
            Vector3 translationOffset)
        {
            var trackingStatus = ERROR_CODE.CAMERA_NOT_DETECTED;
            trackingStatus =
                (ERROR_CODE) dllz_reset_tracking_with_offset(rotation, translation, rotationOffset, translationOffset);
            return trackingStatus;
        }
        ...
        {
            dllz_disable_tracking(new StringBuilder(path, path.Length));
        }

        public ERROR_CODE SaveCurrentArea(string path)
        {
            return (ERROR_CODE) dllz_save_current_area(new StringBuilder(path, path.Length));
        }
        ...
        {
            return (AREA_EXPORT_STATE) dllz_get_area_export_state();
        }
	    ...
	    public Texture2D CreateTextureImageType(VIEW mode, Resolution resolution = new Resolution())
        {
            ...
            var idTexture = m_Texture.GetNativeTexturePtr();
            var error = dllz_register_texture_image_type((int) mode, idTexture, resolution);
            if (error != 0)
                throw new Exception("Error Cuda " + error +
                                    " if the problem appears again, please contact the support");
	    ...
	    public Texture2D CreateTextureMeasureType(MEASURE mode, Resolution resolution = new Resolution())
        {
            ...
            var error = dllz_register_texture_measure_type((int) mode, idTexture, resolution);
	    ...
	    /// <returns></returns>
	    public bool UnregisterTextureImageType(VIEW view)
        {
	    ...
	    /// <param name="view"></param>
	    /// <returns></returns>
	    public bool UnregisterTextureMeasureType(MEASURE measure)
        {
	    ...
	    /// <param name="view"></param>
	    /// <returns></returns>
	    public ZEDMat RequestCopyMatFromTextureImageType(VIEW view)
        {
	    ...
	    /// <param name="view"></param>
	    /// <returns></returns>
	    public ZEDMat RequestCopyMatFromTextureMeasureType(MEASURE measure)
        {
	    ...
	    private void DestroyTexture(int type, int option)
        {
            if (textures.ContainsKey(type) && textures[type].ContainsKey(option))
            {
                ...
                if (textures[type].Count == 0) textures.Remove(type);
            }
        }
	    ...
	    private void DestroyAllTexture()
        {
            if (cameraReady)
            {
                foreach (var t in texturesRequested) DestroyTexture(t.type, t.option);
        ...
        {
            if (HasTexture((int) type, mode)) return textures[(int) type][mode];
            return null;
        }
        ...
        {
            if (cameraReady) return textures.ContainsKey(type) && textures[type].ContainsKey(mode);
            return false;
        }

	    /// <summary>
	    ///     Sets a filtering value for the disparity map (and by extension the depth map). The function should be called before
	    ///     the grab to be taken into account.
	    /// </summary>
	    /// <param name="threshold">
	    ///     a value in [1,100]. A lower value means more confidence and precision (but less density), an upper value reduces
	    ...
	    /// <summary>
	    ///     Set the maximum distance of depth/disparity estimation (all values after this limit will be reported as TOO_FAR
	    ///     value)
	    /// </summary>
        ...
        {
            var p = dllz_get_calibration_parameters(raw);

            if (p == IntPtr.Zero) return new CalibrationParameters();
            var parameters = (CalibrationParameters) Marshal.PtrToStructure(p, typeof(CalibrationParameters));
        ...
        {
            return (MODEL) dllz_get_camera_model();
        }
	    ...
	    ///     The reference type will fill the quaternion and vector with either the diffrences between
	    ///     the last pose(CAMERA) or the cumul of poses (WORLD)
	    /// </param>
	    ...
	    public TRACKING_STATE GetPosition(ref Quaternion rotation, ref Vector3 translation,
            ref Quaternion targetQuaternion, ref Vector3 targetTranslation,
            REFERENCE_FRAME mat_type = REFERENCE_FRAME.WORLD)
        {
            return (TRACKING_STATE) dllz_get_position_at_target_frame(ref rotation, ref translation,
                ref targetQuaternion, ref targetTranslation, (int) mat_type);
        }
	    ...
	    /// </summary>
	    /// <param name="rotation">
	    ///     the quaternion will be filled with the current rotation of the camera depending on the reference
	    ///     and tracking frame
	    /// </param>
	    /// <param name="position">
	    ///     the vector will be filled with the current position of the camera depending on the reference and
	    ///     trancking frameframe
	    /// </param>
	    /// <param name="targetQuaternion"></param>
	    /// <param name="targetTranslation"></param>
	    /// <param name="mat_type"></param>
	    /// <returns></returns>
	    public TRACKING_STATE GetPosition(ref Quaternion rotation, ref Vector3 translation,
            TRACKING_FRAME tracking_frame, REFERENCE_FRAME mat_type = REFERENCE_FRAME.WORLD)
        {
            var rotationOffset = Quaternion.identity;
            var positionOffset = Vector3.zero;
            switch (tracking_frame)
            {
                case TRACKING_FRAME.LEFT_EYE:
                    positionOffset = new Vector3(0, 0, 0);
                    break;
                case TRACKING_FRAME.RIGHT_EYE:
                    positionOffset = new Vector3(Baseline, 0, 0);
                    break;
                case TRACKING_FRAME.CENTER_EYE:
                    positionOffset = new Vector3(Baseline / 2.0f, 0, 0);
                    break;
            }

            return (TRACKING_STATE) dllz_get_position_at_target_frame(ref rotation, ref translation, ref rotationOffset,
                ref positionOffset, (int) mat_type);
        }
	    ...
	    /// <summary>
	    ///     Set a prior to the IMU orientation (only for ZED-M). Prior must come from a external IMU such as the Hmd
	    ///     orientation and must be as much as possible in the same timeframe than the camera
	    /// </summary>
        ...
        {
            var trackingStatus = ERROR_CODE.CAMERA_NOT_DETECTED;
            trackingStatus = (ERROR_CODE) dllz_set_imu_prior_orientation(rotation);
            return trackingStatus;
        }
        ...
        {
            var err = ERROR_CODE.CAMERA_NOT_DETECTED;
            err = (ERROR_CODE) dllz_get_internal_imu_orientation(ref rotation, (int) referenceTime);
            return err;
        }
        ...
        {
            var err = ERROR_CODE.CAMERA_NOT_DETECTED;
            err = (ERROR_CODE) dllz_get_internal_imu_data(ref data, (int) referenceTime);
            return err;
        }
	    ...
	    /// <param name="m"></param>
	    /// <param name="f"></param>
	    public static void Float2Matrix(ref Matrix4x4 m, float[] f)
        {
            if (f == null) return;
	    ...
	    ///     <param name="position">
	    ///         The pixel as a vector3 where the component Z is unused, this position has to be in the
	    ///         screen dimensions "GetDepthValue(Input.mousePosition)" works only if the zed image fills the whole screen
	    ///     </param>
	    ///     <returns>the depth value as a float</returns>
	    /// </summary>
	    public float GetDepthValue(Vector3 pixel)
        {
            if (!cameraReady) return -1;

            var posX = ImageWidth * (pixel.x / Screen.width);
            var posY = ImageHeight * (1 - pixel.y / Screen.height);

            posX = Mathf.Clamp(posX, 0, ImageWidth);
            posY = Mathf.Clamp(posY, 0, ImageHeight);
            var d = dllz_get_depth_value((uint) posX, (uint) posY);
            return d;
        }
	    ...
	    ///     <param name="pixel">
	    ///         The pixel as a vector3 where the component Z is unused, this position has to be in the screen
	    ///         dimensions "GetDistancehValue(Input.mousePosition)" works only if the zed image fills the whole screen
	    ///     </param>
	    ...
	    ///     <param name="pixel">
	    ///         The pixel as a vector3 where the component Z is unused, this position has to be in the screen
	    ///         dimensions "GetNormalValue(Input.mousePosition)" works only if the zed image fills the whole screen
	    ///     </param>
	    ...
	    public bool GETXYZValue(Vector3 pixel, out Vector4 xyz)
        {
            ...
            posX = Mathf.Clamp(posX, 0, ImageWidth);
            posY = Mathf.Clamp(posY, 0, ImageHeight);
            var r = dllz_get_xyz_value((uint) posX, (uint) posY, out xyz) != 0;
            return r;
        }
	    ...
	    ///     The normal is in the camera frame. Use cam.worldToCameraMatrix.inverse to transform to world normals. Or use the
	    ///     ZEDSupportFunctions
	    ///     <param name="pixel">
	    ///         The pixel as a vector3, the component Z is unused, this position has to be in the screen
	    ///         dimensions "GetNormalValue(Input.mousePosition)" works only if the zed image fills the whole screen
	    ///     </param>
	    ...
	    public bool GetNormalValue(Vector3 pixel, out Vector4 normal)
        {
            ...
            var r = dllz_get_normal_value((uint) posX, (uint) posY, out normal) != 0;
            return r;
        }
	    ...
	    /// <returns></returns>
	    public ERROR_CODE EnableSpatialMapping(float resolution_meter, float max_range_meter, bool saveTexture = false)
        {
            var spatialMappingStatus = ERROR_CODE.FAILURE;
            lock (ZEDManager.Instance.grabLock)
            {
                spatialMappingStatus = (ERROR_CODE) dllz_enable_spatial_mapping(resolution_meter, max_range_meter,
                    Convert.ToInt32(saveTexture));
            }
	    ...
	    /// <returns></returns>
	    public ERROR_CODE UpdateMesh(int[] nbVerticesInSubemeshes, int[] nbTrianglesInSubemeshes, ref int nbSubmeshes,
            int[] updatedIndices, ref int nbVertices, ref int nbTriangles, int nbSubmeshMax)
        {
            var err = ERROR_CODE.FAILURE;
            err = (ERROR_CODE) dllz_update_mesh(nbVerticesInSubemeshes, nbTrianglesInSubemeshes, ref nbSubmeshes,
                updatedIndices, ref nbVertices, ref nbTriangles, nbSubmeshMax);
            return err;
        }

	    /// <summary>
	    ///     Retrieves all the chunks of the generated mesh. You need to call UpdateMesh first. vertices and triangles must be
	    ///     at least of the size returned by UpdateMesh (nbVertices and nbTriangles).
	    /// </summary>
	    ...
	    /// <param name="nbSubmeshMax">Maximal number of submeshes handle in the script.</param>
	    /// <returns></returns>
	    public ERROR_CODE RetrieveMesh(Vector3[] vertices, int[] triangles, int nbSubmeshMax, Vector2[] uvs,
            IntPtr textures)
        {
            return (ERROR_CODE) dllz_retrieve_mesh(vertices, triangles, nbSubmeshMax, uvs, textures);
        }
	    ...
	    public ERROR_CODE GetMeshRequestStatus()
        {
            return (ERROR_CODE) dllz_get_mesh_request_status_async();
        }
	    ...
	    /// <returns></returns>
	    public bool LoadMesh(string filename, int[] nbVerticesInSubemeshes, int[] nbTrianglesInSubemeshes,
            ref int nbSubmeshes, int[] updatedIndices, ref int nbVertices, ref int nbTriangles, int nbSubmeshMax,
            int[] textureSize = null)
        {
            return dllz_load_mesh(filename, nbVerticesInSubemeshes, nbTrianglesInSubemeshes, ref nbSubmeshes,
                updatedIndices, ref nbVertices, ref nbTriangles, nbSubmeshMax, textureSize);
        }
	    ...
	    public bool FilterMesh(FILTER filterParameters, int[] nbVerticesInSubemeshes, int[] nbTrianglesInSubemeshes,
            ref int nbSubmeshes, int[] updatedIndices, ref int nbVertices, ref int nbTriangles, int nbSubmeshMax)
        {
            return dllz_filter_mesh(filterParameters, nbVerticesInSubemeshes, nbTrianglesInSubemeshes, ref nbSubmeshes,
                updatedIndices, ref nbVertices, ref nbTriangles, nbSubmeshMax);
        }
	    ...
	    /// <param name="nbTrianglesInSubemeshes">
	    ///     Vector containing the number of triangles in each textured submeshes after
	    ///     filtering.
	    /// </param>
	    /// <param name="nbSubmeshes"> Number of submeshes after filtering. </param>
	    ...
	    /// <param name="nbSubmeshMax"> Maximal number of submeshes handle in the script. </param>
	    /// <returns></returns>
	    public bool ApplyTexture(int[] nbVerticesInSubemeshes, int[] nbTrianglesInSubemeshes, ref int nbSubmeshes,
            int[] updatedIndices, ref int nbVertices, ref int nbTriangles, int[] textureSize, int nbSubmeshMax)
        {
            return dllz_apply_texture(nbVerticesInSubemeshes, nbTrianglesInSubemeshes, ref nbSubmeshes, updatedIndices,
                ref nbVertices, ref nbTriangles, textureSize, nbSubmeshMax);
        }
        ...
        {
            return (SPATIAL_MAPPING_STATE) dllz_get_spatial_mapping_state();
        }
	    ...
	    public void MergeChunks(int numberFaces, int[] nbVerticesInSubemeshes, int[] nbTrianglesInSubemeshes,
            ref int nbSubmeshes, int[] updatedIndices, ref int nbVertices, ref int nbTriangles, int nbSubmesh)
        {
            dllz_spatial_mapping_merge_chunks(numberFaces, nbVerticesInSubemeshes, nbTrianglesInSubemeshes,
	    ...
	    /// <returns></returns>
	    public ERROR_CODE RetrieveMeasure(ZEDMat mat, MEASURE measure, ZEDMat.MEM mem = ZEDMat.MEM.MEM_CPU,
            Resolution resolution = new Resolution())
        {
            return (ERROR_CODE) dllz_retrieve_measure(mat.MatPtr, (int) measure, (int) mem, resolution);
        }
	    ...
	    /// <param name="resolution"></param>
	    /// <returns></returns>
	    public ERROR_CODE RetrieveImage(ZEDMat mat, VIEW view, ZEDMat.MEM mem = ZEDMat.MEM.MEM_CPU,
            Resolution resolution = new Resolution())
        {
        ...
        {
            var p = IntPtr.Zero;
            var calib = GetCalibrationParameters(false); //GetCalibrationinstance.CalibrationParametersRectified;
            var imageResolution = new Resolution((uint) instance.ImageWidth, (uint) instance.ImageHeight);


            var calibLeft = new Vector4(calib.leftCam.fx, calib.leftCam.fy, calib.leftCam.cx, calib.leftCam.cy);
            var calibRight = new Vector4(calib.rightCam.fx, calib.rightCam.fy, calib.rightCam.cx, calib.rightCam.cy);

            p = dllz_compute_optical_center_offsets(ref calibLeft, ref calibRight, imageResolution, planeDistance);

            if (p == IntPtr.Zero) return new Vector4();
            var parameters = (Vector4) Marshal.PtrToStructure(p, typeof(Vector4));
            return parameters;
        }
        ...
        ////*
	    /// Plane Detection
	    public ERROR_CODE findFloorPlane(ref ZEDPlaneGameObject.PlaneData plane, out float playerHeight,
            Quaternion priorQuat, Vector3 priorTrans)
        {
            var p = IntPtr.Zero;
            var out_quat = Quaternion.identity;
            var out_trans = Vector3.zero;
        ...
        public ERROR_CODE findPlaneAtHit(ref ZEDPlaneGameObject.PlaneData plane, Vector2 screenPos)
        {
            var p = IntPtr.Zero;
            var out_quat = Quaternion.identity;
            var out_trans = Vector3.zero;

            var posX = ImageWidth * (screenPos.x / Screen.width);
            var posY = ImageHeight * (1 - screenPos.y / Screen.height);
            posX = Mathf.Clamp(posX, 0, ImageWidth);
            posY = Mathf.Clamp(posY, 0, ImageHeight);
        ...
        public struct dll_initParameters
        {
            ...
            [MarshalAs(UnmanagedType.U1)] public bool cameraImageFlip;
            [MarshalAs(UnmanagedType.U1)] public bool enableRightSideMeasure;
            [MarshalAs(UnmanagedType.U1)] public bool cameraDisableSelfCalib;
