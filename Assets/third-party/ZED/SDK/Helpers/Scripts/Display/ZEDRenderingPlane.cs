...
public class ZEDRenderingPlane : MonoBehaviour
{
    ...
    /// </summary>
    private readonly float aspect = 16.0f / 9.0f;
    ...
    /// </summary>
    private readonly CommandBuffer[] buffer = new CommandBuffer[(int) ZED_RENDERING_MODE.LAST];
    ...
    private ComputeBuffer computeBufferSpotLight;
    ...
    /// </summary>
    private readonly Vector4[] directionalLightData = new Vector4[2]; //Limit to one directional light


    //Invisible object to force unity to render shadow map
    private GameObject forceShadowObject;

    /// <summary>
    ///     The material used to integrate the depth in forward mode after the depth texture is created, mainly use to get the
    ///     shadows. Not needed for the light only
    /// </summary>
    private Material forwardMat;

    private bool ghasShadows;
    ...
    /// </summary>
    [HideInInspector] public int numberPointLights;

    //Forward ID shader cache
    private int numberPointLightsID;

    [HideInInspector] public int numberSpotLights;
    ...
    /// </summary>
    [SerializeField] public PointLight[] pointLights = new PointLight[NUMBER_POINT_LIGHT_MAX];

    /// <summary>
    ///     CommandBuffer to create a mask for the virtual objects in forward and deferred
    /// </summary>
    private readonly CommandBuffer[] postProcessBuffer = new CommandBuffer[(int) ZED_RENDERING_MODE.LAST];
    ...
    /// </summary>
    [HideInInspector] private RenderTexture renderTextureTarget;

    /// <summary>
    ///     A Low resolution of the textures from the ZED
    /// </summary>
    private readonly Resolution resolution = new Resolution(384, 192);
    ...
    [SerializeField] public SpotLight[] spotLights = new SpotLight[NUMBER_SPOT_LIGHT_MAX];
    ...
    ///     View mode
    /// </summary>
    public VIEW_MODE viewMode = VIEW_MODE.VIEW_IMAGE;
    ...
    public Texture2D TextureEye => textureEye;

    public RenderingPath ActualRenderingPath => actualRenderingPath;

    public RenderTexture target => renderTextureTarget;
    ...
    private void Start()
    {
        ...
        postprocessMaterial =
            new Material(Resources.Load("Materials/PostProcessing/Mat_ZED_PostProcessing") as Material);
    ...
    private void ZEDReady()
    {
        ...
        var plane_distance = 0.15f;
        var opticalCenters = zedCamera.ComputeOpticalCenterOffsets(plane_distance);
    ...
    {
        var current = actualRenderingPath;
        ...
        else
        {
            ...
            Debug.LogError(" [ ZED Plugin ] : The rendering path " + mainCamera.actualRenderingPath +
                           " is not compatible with the ZED");
        }
    }
    ...
    private void SetForward()
    {
        ...
        matRGB.SetInt("_isLinear", Convert.ToInt32(QualitySettings.activeColorSpace));

        forwardMat = new Material(Resources.Load("Materials/Lighting/Mat_ZED_Forward") as Material);


        // Configure the invisible object
        if (forceShadowObject == null) ConfigureForceShadowObject();
        ...
        if (postProcessBuffer[(int) ZED_RENDERING_MODE.DEFERRED] != null)
            mainCamera.RemoveCommandBuffer(CameraEvent.AfterFinalPass,
                postProcessBuffer[(int) ZED_RENDERING_MODE.DEFERRED]);
        ...
        buffer[(int) ZED_RENDERING_MODE.FORWARD].DrawMesh(meshCanvas.mesh,
            gameObject.transform.GetChild(0).transform.localToWorldMatrix, forwardMat);
        ...
        postProcessBuffer[(int) ZED_RENDERING_MODE.FORWARD]
            .Blit(BuiltinRenderTextureType.CameraTarget, mask, blitMaterial, 0);
        postProcessBuffer[(int) ZED_RENDERING_MODE.FORWARD].SetGlobalTexture("_ZEDMaskVirtual", mask);

        mainCamera.RemoveCommandBuffer(CameraEvent.AfterForwardAlpha,
            postProcessBuffer[(int) ZED_RENDERING_MODE.FORWARD]);
    ...
    private void SetDeferred()
    {
        ...
        GraphicsSettings.SetCustomShader(BuiltinShaderType.DeferredShading,
            (Resources.Load("Materials/Lighting/Mat_ZED_Deferred_Lighting") as Material).shader);
        ...
        if (mainCamera.allowHDR)
        {
            ...
            {
                BuiltinRenderTextureType.GBuffer0, BuiltinRenderTextureType.GBuffer1, BuiltinRenderTextureType.GBuffer2,
        ...
        buffer[(int) ZED_RENDERING_MODE.DEFERRED].DrawMesh(meshCanvas.mesh,
            gameObject.transform.GetChild(0).transform.localToWorldMatrix, deferredMat);
        ...
        postProcessBuffer[(int) ZED_RENDERING_MODE.DEFERRED]
            .Blit(BuiltinRenderTextureType.GBuffer0, mask, blitMaterial, 0);
    ...
    private void ConfigureForceShadowObject()
    {
        ...
        forceShadowObject.GetComponent<MeshRenderer>().sharedMaterial =
            Resources.Load("Materials/Lighting/Mat_ZED_Hide") as Material;
    ...
    private void CreateRenderTexture()
    {
        ...
        if (mainCamera.stereoTargetEye != StereoTargetEyeMask.None)
        {
            ...
            {
                renderTextureTarget = new RenderTexture(zedCamera.ImageWidth, zedCamera.ImageHeight, 24,
                    RenderTextureFormat.ARGB32);
                mainCamera.targetTexture = renderTextureTarget;
            }
            else if (ZEDCamera.CheckPlugin())
            {
    ...
    private void SetTextures(ZEDCamera zedCamera, VIEW_MODE view_mode)
    {
        var baseline = zedCamera.Baseline;
        ...
        if (StereoTargetEyeMask.Left == mainCamera.stereoTargetEye)
        {
            ...
            switch (view_mode)
            {
                case VIEW_MODE.VIEW_IMAGE:
                    textureEye = zedCamera.CreateTextureImageType(VIEW.LEFT);
                    break;
                case VIEW_MODE.VIEW_DEPTH:
                    textureEye = zedCamera.CreateTextureImageType(VIEW.DEPTH);
                    break;
                case VIEW_MODE.VIEW_NORMALS:
                    textureEye = zedCamera.CreateTextureImageType(VIEW.NORMALS);
                    break;
            }

            normals = zedCamera.CreateTextureMeasureType(MEASURE.NORMALS, resolution);
            depth = zedCamera.CreateTextureMeasureType(MEASURE.DEPTH, resolution);
        }
        else if (StereoTargetEyeMask.Right == mainCamera.stereoTargetEye)
        {
            ...
            {
                case VIEW_MODE.VIEW_IMAGE:
                    textureEye = zedCamera.CreateTextureImageType(VIEW.RIGHT);
                    break;
                case VIEW_MODE.VIEW_DEPTH:
                    textureEye = zedCamera.CreateTextureImageType(VIEW.DEPTH_RIGHT);
                    break;
                case VIEW_MODE.VIEW_NORMALS:
                    textureEye = zedCamera.CreateTextureImageType(VIEW.NORMALS_RIGHT);
                    break;
            }

            normals = zedCamera.CreateTextureMeasureType(MEASURE.NORMALS_RIGHT, resolution);
            depth = zedCamera.CreateTextureMeasureType(MEASURE.DEPTH_RIGHT, resolution);
        }
        else
        {
            side = 0;
    ...
    private void UpdateLights()
    {
        ...
        foreach (var zed_light in ZEDLight.s_lights)
        {
            var light = zed_light.cachedLight;

            if (light.type == LightType.Directional || Vector3.Distance(bounds.center, light.transform.position) <
                light.range + bounds.extents.x)
            {
                ...
                if (zed_light.IsEnabled() && (pointLightIndex + spotLighIndex < NUMBER_LIGHTS_MAX ||
                                              light.type == LightType.Directional))
                {
                    ...
                    else if (light.type == LightType.Spot)
                    {
                        if (spotLighIndex < NUMBER_SPOT_LIGHT_MAX)
                        {
                            ...
                            spotLights[spotLighIndex].direction = new Vector4(
                                light.gameObject.transform.forward.normalized.x,
                                light.gameObject.transform.forward.normalized.y,
                                light.gameObject.transform.forward.normalized.z,
                                Mathf.Cos(light.spotAngle / 2.0f * Mathf.Deg2Rad));
                            spotLights[spotLighIndex].parameters = new Vector4(light.spotAngle, light.intensity,
                                1.0f / light.range, zed_light.interiorCone);
                            spotLighIndex++;
                        }
                    }
                    else if (light.type == LightType.Directional)
                    {
                        hasShadows = light.shadows != LightShadows.None &&
                                     QualitySettings.shadows != ShadowQuality.Disable;
                        directionalLightData[0] = new Vector4(light.gameObject.transform.forward.normalized.x,
                            light.gameObject.transform.forward.normalized.y,
                            light.gameObject.transform.forward.normalized.z, 0);
                        ...
                        {
                            forwardMat.SetInt("_HasShadows", Convert.ToInt32(light.shadows != LightShadows.None));

                            // Copy the shadows from the directional light, If not no shadows in transparent mode
                            if (light.commandBufferCount == 0)
                            {
                                var lightBuffer = new CommandBuffer();
                                lightBuffer.name = "ZED_Copy_ShadowMap";
                                lightBuffer.SetGlobalTexture("_DirectionalShadowMap",
        ...
        //Upload the new Data
        if (computeBuffePointLight != null) computeBuffePointLight.SetData(pointLights);
        if (computeBufferSpotLight != null) computeBufferSpotLight.SetData(spotLights);
        ...
        if (matRGB != null)
        {
            ...
            if (hasShadows != ghasShadows)
            {
                ...
                Shader.SetGlobalInt("_HasShadows", Convert.ToInt32(ghasShadows));
                mainCamera.RemoveCommandBuffer(CameraEvent.BeforeDepthTexture,
                    buffer[(int) ZED_RENDERING_MODE.FORWARD]);
                if (hasShadows)
                    mainCamera.AddCommandBuffer(CameraEvent.BeforeDepthTexture,
                        buffer[(int) ZED_RENDERING_MODE.FORWARD]);
            }
    ...
    /// <returns></returns>
    private float GetFOVYFromProjectionMatrix(Matrix4x4 projection)
    {
        return Mathf.Atan(1 / projection[1, 1]) * 2.0f;
    }

    private float GetFOVXFromProjectionMatrix(Matrix4x4 projection)
    {
    ...
    {
        var height = Mathf.Tan(0.5f * fov) * Vector3.Distance(screen.transform.localPosition, Vector3.zero) * 2;
    ...
    {
        var height = Mathf.Tan(0.5f * fovV) * Vector3.Distance(screen.transform.localPosition, Vector3.zero) * 2;
        var width = Mathf.Tan(0.5f * fovH) * Vector3.Distance(screen.transform.localPosition, Vector3.zero) * 2;
    ...
    {
        if (computeBuffePointLight != null) computeBuffePointLight.Release();

        if (computeBufferSpotLight != null) computeBufferSpotLight.Release();
    ...
    private void Update()
    {
        if (aspectRatio != null) aspectRatio.Update();
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        {
            var tmpSource = RenderTexture.GetTemporary(source.width, source.height, source.depth, source.format,
        ...
        else
        {
            ...
            if (ARpostProcessing && mask != null && zedCamera.IsCameraReady)
            {
                ...
                {
                    var bluredMask = RenderTexture.GetTemporary(mask.width, mask.height, mask.depth, mask.format);
                    var buffer = RenderTexture.GetTemporary(source.width, source.height, 24);
    ...
    {
        var tempDestination = RenderTexture.GetTemporary(source.width, source.height, source.depth,
    ...
    /// </summary>
    private enum ZED_RENDERING_MODE
    {
        FORWARD,
        DEFERRED,
        LAST
    ...
    public class WindowAspectRatio
    {
        ...
        {
            var o = new GameObject("CameraBlackBackground");
