...
public class ZEDSVOManager : MonoBehaviour
{
    ...
    /// </summary>
    [SerializeField] public SVO_COMPRESSION_MODE compressionMode = SVO_COMPRESSION_MODE.LOSSY_BASED;
    ...
    /// </summary>
    [SerializeField] [Tooltip("Loop the SVO")]
    public bool loop;
    ...
    /// </summary>
    [HideInInspector] [SerializeField] private int numberFrameMax;

    /// <summary>
    ///     Pause the SVO, Unity will continue to run
    /// </summary>
    [HideInInspector] [SerializeField] public bool pause;

    /// <summary>
    ///     Set to true to read a SVO, a SVO cannot be read and written at once
    /// </summary>
    [SerializeField] public bool read;

    [SerializeField] [HideInInspector] public bool realtimePlayback = true;
    ...
    /// </summary>
    [SerializeField] [HideInInspector] public string videoFile = "Assets/ZEDRecording.svo";
    ...
    public void InitSVO(ZEDCamera zedCamera)
    {
        if (record)
            if (zedCamera.EnableRecording(videoFile, compressionMode) != ERROR_CODE.SUCCESS)
                record = false;

        if (read) NumberFrameMax = zedCamera.GetSVONumberOfFrames();
    }
}
...
[CustomEditor(typeof(ZEDSVOManager))]
[CanEditMultipleObjects]
public class SVOManagerInspector : Editor
{
    private bool current_readValue;
    private bool current_recordValue;
    private SerializedProperty currentFrame;

    private Rect drop_area;

    private readonly string[] filters = {"Svo files", "svo"};
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            var tooltip =
                "Uses timecodes of each frame for playback. Causes a pause for dropped frames or when pause was used during recording.";
            obj.realtimePlayback =
                EditorGUILayout.Toggle(new GUIContent("Realtime Playback", tooltip), obj.realtimePlayback);
        }
        ...
        if (EditorGUI.EndChangeCheck()) CheckChange();
        EditorGUI.BeginChangeCheck();
        ...
        if (EditorGUI.EndChangeCheck())
            if (ZEDCamera.GetInstance() != null)
            {
                //If the slider of frame from the SVO has moved, grab mannually the frame and update the textures
                ZEDCamera.GetInstance().SetSVOPosition(currentFrame.intValue);
                if (pause.boolValue) obj.NeedNewFrameGrab = true;
            }

        GUI.enabled = true;

        GUI.enabled = ZEDCamera.GetInstance() != null && ZEDCamera.GetInstance().IsCameraReady;
        pauseText = pause.boolValue ? "Resume" : "Pause";
        if (GUILayout.Button(pauseText)) pause.boolValue = !pause.boolValue;
        GUI.enabled = true;
        DropAreaGUI();
    ...
    {
        if (loop.boolValue && record.boolValue) loop.boolValue = false;
        ...
        if (!read.boolValue && current_readValue != read.boolValue) loop.boolValue = false;
        ...
        switch (evt.type)
        {
            ...
            case EventType.DragPerform:
                ...
                if (evt.type == EventType.DragPerform)
                {
                    ...
                    foreach (var dragged_object in DragAndDrop.paths) videoFile.stringValue = dragged_object;
                }
