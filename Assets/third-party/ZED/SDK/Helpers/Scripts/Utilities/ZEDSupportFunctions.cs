...
public class ZEDSupportFunctions
{
	...
	/// <summary>
	///     Get the Normal vector at a given pixel (i,j). the Normal can be given regarding camera reference or in the world
	///     reference.
	/// </summary>
	...
	public static bool GetNormalAtPixel(Vector2 pixel, REFERENCE_FRAME reference_frame, Camera cam, out Vector3 normal)
    {
        Vector4 n;
        var r = ZEDCamera.GetInstance().GetNormalValue(new Vector3(pixel.x, pixel.y, 0), out n);
	...
	/// <summary>
	///     Get the Normal vector at a world position (x,y,z). the Normal can be given regarding camera reference or in the
	///     world reference.
	/// </summary>
	...
	/// <returns> true if success, false otherwie</returns>
	public static bool GetNormalAtWorldLocation(Vector3 position, REFERENCE_FRAME reference_frame, Camera cam,
        out Vector3 normal)
    {
        Vector4 n;
        var r = ZEDCamera.GetInstance().GetNormalValue(cam.WorldToScreenPoint(position), out n);
    ...
    {
        var d = ZEDCamera.GetInstance().GetDepthValue(new Vector3(pixel.x, pixel.y, 0));
        depth = d;
    ...
    {
        var pixelPosition = cam.WorldToScreenPoint(position);

        var d = ZEDCamera.GetInstance().GetDepthValue(new Vector3(pixelPosition.x, pixelPosition.y, 0));
        depth = d;
    ...
    {
        var d = ZEDCamera.GetInstance().GetDistanceValue(new Vector3(pixel.x, pixel.y, 0));
        distance = d;
	...
	public static bool GetEuclideanDistanceAtWorldLocation(Vector3 position, Camera cam, out float distance)
    {
        ...
        var d = ZEDCamera.GetInstance().GetDistanceValue(new Vector3(pixelPosition.x, pixelPosition.y, 0));
        distance = d;
	...
	public static bool GetWorldPositionAtPixel(Vector2 pixel, Camera cam, out Vector3 worldPos)
    {
        ...
        var xp = pixel.x * ZEDCamera.GetInstance().ImageWidth / Screen.width;
        var yp = pixel.y * ZEDCamera.GetInstance().ImageHeight / Screen.height;
	...
	public static bool IsPixelVisible(Vector2 pixel, Camera cam)
    {
        ...
        var position = cam.ScreenToWorldPoint(new Vector3(pixel.x, pixel.y, d));
        if (Physics.Raycast(cam.transform.position, position - cam.transform.position, out hit))
            if (hit.distance < d)
                return false;
        return true;
	...
	///     In each function, "countinvalidascollision" specifies if off-screen pixels or missing depth values should count as
	///     collision.
	///     "realworldthickness" specifies how far back a point needs to be behind the real world before it's not considered a
	///     collision.
	/// </summary>
	/// <summary>
	...
	public static bool HitTestAtPoint(Camera camera, Vector3 point, bool countinvalidascollision = false,
        float realworldthickness = Mathf.Infinity)
    {
        ...
        if (!CheckScreenView(point, camera)) return countinvalidascollision;
        ...
        if (realdistance <= 0f) return countinvalidascollision; //We can't read the depth from that pixel.

        ///Detection is the space
        if (realdistance <= Vector3.Distance(point, camera.transform.position) &&
            Vector3.Distance(point, camera.transform.position) - realdistance <= realworldthickness)
	...
	/// <returns></returns>
	public static bool HitTestOnRay(Camera camera, Vector3 startpos, Quaternion rot, float maxdistance,
        float distbetweendots, out Vector3 collisionpoint,
        bool countinvalidascollision = false, float realworldthickness = Mathf.Infinity)
    {
        ...
        var lastvalidpoint = startpos;
        for (float i = 0; i < maxdistance; i += distbetweendots)
        {
            var pointtocheck = rot * new Vector3(0f, 0f, i);
            pointtocheck += startpos;

            var hit = HitTestAtPoint(camera, pointtocheck, countinvalidascollision, realworldthickness);
	...
	/// <summary>
	///     Checks if a spherical area is blocked above a given percentage. Useful for checking is a drone spawn point is
	///     valid.
	///     Works by checking random points around the sphere for occlusion, Monte Carlo-style, so more samples means greater
	///     accuracy.
	/// </summary>
	/// <param name="camera">camera (Left camera usually)</param>
	...
	/// <returns></returns>
	public static bool HitTestOnSphere(Camera camera, Vector3 centerpoint, float radius, int numberofsamples,
        float blockedpercentagethreshold,
        bool countinvalidascollision = true, float realworldthickness = Mathf.Infinity)
    {
        var occludedpoints = 0;

        for (var i = 0; i < numberofsamples; i++)
        {
            //Find a random point along the bounds of a sphere and check if it's occluded
            var randompoint = Random.onUnitSphere * radius + centerpoint;
            if (HitTestAtPoint(camera, randompoint, countinvalidascollision, realworldthickness)) occludedpoints++;
        }

        //See if the percentage of occluded pixels exceeds the threshold
        var occludedpercent = occludedpoints / (float) numberofsamples;
	...
	/// <returns></returns>
	public static bool HitTestOnMeshVertices(Camera camera, Mesh mesh, Transform worldtransform,
        float blockedpercentagethreshold, float meshsamplepercent = 1,
        bool countinvalidascollision = false, float realworldthickness = Mathf.Infinity)
    {
        //Find how often we check samples, represented as an integer denominator
        var checkfrequency = Mathf.RoundToInt(1f / Mathf.Clamp01(meshsamplepercent));
        var totalchecks = Mathf.FloorToInt(mesh.vertices.Length / (float) checkfrequency);

        //Check the vertices in the mesh for a collision, skipping vertices to match the specified sample percentage.
        var intersections = 0;
        for (var i = 0; i < mesh.vertices.Length; i += checkfrequency)
            if (HitTestAtPoint(camera, worldtransform.TransformPoint(mesh.vertices[i]), countinvalidascollision,
                realworldthickness))
                intersections++;

        //See if our total collisions exceeds the threshold to call it a collision.
        var blockedpercentage = (float) intersections / totalchecks;
        if (blockedpercentage > blockedpercentagethreshold) return true;
	...
	/// <param name="realworldthickness">see HitTestAtPoint</param>
	/// <returns></returns>
	public static bool HitTestOnMeshFilter(Camera camera, MeshFilter meshfilter, float blockedpercentagethreshold,
        float meshsamplepercent = 1,
        bool countinvalidascollision = false, float realworldthickness = Mathf.Infinity)
    {
        return HitTestOnMeshVertices(camera, meshfilter.mesh, meshfilter.transform, blockedpercentagethreshold,
            meshsamplepercent, countinvalidascollision, realworldthickness);
    }
	...
	public static bool CheckScreenView(Vector3 point, Camera camera)
    {
        ...
        //Make sure it's within our view frustrum (except for clipping planes)
        if (screenpoint.z <= 0f) return false; //No collision if it's behind us.
    ...
    public static bool SaveImage(RenderTexture rt, string path = "Assets/image.png")
    {
