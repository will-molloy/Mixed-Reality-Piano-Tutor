...
public class ZEDPlaneGameObject : MonoBehaviour
{
    ...
    public Vector3 worldCenter => gameObject.transform.position;
    ...
    private void SetComponents(PlaneData plane, Vector3[] vertices, int[] triangles, Material rendermaterial)
    {
        ...
        var highestvertindex = 0;
        for (var i = 0; i < triangles.Length; i++)
            if (triangles[i] > highestvertindex)
                highestvertindex = triangles[i];
        Array.Resize(ref vertices, highestvertindex + 1);


        //Calculate the UVs for the vertices based on world space so they line up with other planes
        var uvs = new Vector2[vertices.Length];
        var rotatetobackward = Quaternion.FromToRotation(worldNormal, Vector3.back);
        for (var i = 0; i < vertices.Length; i++)
        {
            var upwardcoords = rotatetobackward * (vertices[i] + worldCenter);
    ...
    public void Create(PlaneData plane, Vector3[] vertices, int[] triangles, int opt_count, Material rendermaterial)
    {
        ...
        Array.Copy(plane.Bounds, planeData.Bounds, plane.BoundsSize);
        ...
        gameObject.layer = ZEDCamera.TagOneObject;
    ...
    /// <param name="force">
    ///     If set to <c>true</c> force the update. Is set to false, update only if new plane/mesh is bigger or
    ///     contains the old one
    /// </param>
    /// <param name="plane">Plane.</param>
    /// <param name="vertices">Vertices.</param>
    /// <param name="triangles">Triangles.</param>
    public bool UpdateFloorPlane(bool force, PlaneData plane, Vector3[] vertices, int[] triangles,
        Material rendermaterial = null)
    {
        ...
        if (!force)
        {
            ...
            else
            {
                ...
                var tmpNBnds = tmp.bounds;
                var mf = gameObject.GetComponent<MeshFilter>();

                if (tmpNBnds.Contains(mf.mesh.bounds.min) && tmpNBnds.Contains(mf.mesh.bounds.max) ||
                    tmpNBnds.size.x * tmpNBnds.size.y > 1.1 * mf.mesh.bounds.size.x * mf.mesh.bounds.size.y)
                    need_update = true;
    ...
    private Material GetDefaultMaterial(PLANE_TYPE type)
    {
        ...
        var defaultmaterial =
            new Material(Resources.Load("Materials/PlaneDetection/Mat_ZED_Geometry_WirePlane") as Material);
        switch (type)
        {
            case PLANE_TYPE.FLOOR:
                //Floor planes are blue
                defaultmaterial.SetColor("_WireColor",
                    new Color(44.0f / 255.0f, 157.0f / 255.0f, 222.0f / 255.0f, 174.0f / 255.0f));
                break;
            ...
            case PLANE_TYPE.HIT_UNKNOWN:
                // Hit planes are pink
                defaultmaterial.SetColor("_WireColor",
                    new Color(221.0f / 255.0f, 20.0f / 255.0f, 149.0f / 255.0f, 174.0f / 255.0f));
                break;
