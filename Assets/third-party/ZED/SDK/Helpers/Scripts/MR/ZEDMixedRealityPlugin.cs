...
public class ZEDMixedRealityPlugin : MonoBehaviour
{
    ...
    private static extern IntPtr dllz_compute_size_plane_with_gamma(Resolution resolution, float perceptionDistance,
        float eyeToZedDistance, float planeDistance, float HMDFocal, float zedFocal);

    [DllImport(nameDll, EntryPoint = "dllz_compute_hmd_focal")]
    private static extern float dllz_compute_hmd_focal(Resolution r, float w, float h);

    /*****LATENCY CORRECTOR***/
    [DllImport(nameDll, EntryPoint = "dllz_latency_corrector_add_key_pose")]
    private static extern void dllz_latency_corrector_add_key_pose(ref Vector3 translation, ref Quaternion rotation,
        ulong timeStamp);

    [DllImport(nameDll, EntryPoint = "dllz_latency_corrector_get_transform")]
    private static extern int dllz_latency_corrector_get_transform(ulong timeStamp, bool useLatency,
    ...
    public static extern void dllz_drift_corrector_get_tracking_data(ref TrackingData trackingData,
        ref Pose HMDTransform, ref Pose latencyCorrectorTransform, int hasValidTrackingPosition, bool checkDrift);
	...
	/// </summary>
	public Vector3 offset = new Vector3(0, 0, (float) Constant.PLANE_DISTANCE);
	...
	/// </summary>
	public bool zedReady;

	/// <summary>
	///     Flag is VRDevice is detected. Updated at every frame
	/// </summary>
	private bool hasVRDevice;

    public bool HasVRDevice => hasVRDevice;
    ...
    public Pose HmdToZEDCalibration => hmdtozedCalibration;

	/// <summary>
	///     Check if the latency correction is ready
	/// </summary>
	private bool latencyCorrectionReady;
    ...
    [SerializeField] private string calibrationFile = "CalibrationZEDHMD.ini";
    ...
    private void Awake()
    {
        hasVRDevice = XRDevice.isPresent;
        if (hasVRDevice)
        {
            if (XRDevice.model.ToLower().Contains("vive"))
                dllz_latency_corrector_initialize(0);
            else if (XRDevice.model.ToLower().Contains("oculus"))
	...
	private void Start()
    {
        ...
        var plane_dist = (float) Constant.PLANE_DISTANCE;
	...
	/// <returns></returns>
	public Vector2 ComputeSizePlaneWithGamma(Resolution resolution, float perceptionDistance, float eyeToZedDistance,
        float planeDistance, float HMDFocal, float zedFocal)
    {
        var p = dllz_compute_size_plane_with_gamma(resolution, perceptionDistance, eyeToZedDistance, planeDistance,
            HMDFocal, zedFocal);

        if (p == IntPtr.Zero) return new Vector2();
        var parameters = (Vector2) Marshal.PtrToStructure(p, typeof(Vector2));
        return parameters;
    }
	...
	/// <returns></returns>
	public float ComputeFocal(Resolution targetSize)
    {
        var focal_hmd = dllz_compute_hmd_focal(targetSize, finalLeftEye.projectionMatrix.m00,
            finalLeftEye.projectionMatrix.m11);
        return focal_hmd;
    }
    ...
    private void ZEDReady()
    {
        ...
        var perception_distance = 1.0f;
        var zed2eye_distance = 0.1f;
        hasVRDevice = XRDevice.isPresent;

        if (hasVRDevice)
        {
            var parameters = zedCamera.CalibrationParametersRectified;

            scaleFromZED = ComputeSizePlaneWithGamma(
                new Resolution((uint) zedCamera.ImageWidth, (uint) zedCamera.ImageHeight),
                perception_distance, zed2eye_distance, offset.z,
                ComputeFocal(new Resolution((uint) XRSettings.eyeTextureWidth,
                    (uint) XRSettings.eyeTextureHeight)), //571.677612f,
        ...
        if (XRDevice.model.ToLower().Contains("vive"))
        {
            zedCamera.SetCameraSettings(CAMERA_SETTINGS.CONTRAST, 3);
            zedCamera.SetCameraSettings(CAMERA_SETTINGS.SATURATION, 3);
        }
        ...
        if (zedCamera.ImageWidth != 1280 && zedCamera.ImageHeight != 720)
            Debug.LogWarning(
                "[ZED AR Passthrough] This resolution is not compatible with proper AR passthrough experience");
	...
	public void CollectPose()
    {
        ...
        {
            k.Timestamp = ZEDCamera.GetInstance().GetCurrentTimeStamp();
            if (k.Timestamp >= 0)
	...
	public Pose InitTrackingAR()
    {
        ...
        var t = Vector3.zero;
        var const_offset = new Pose(t, r);
        dllz_drift_corrector_set_calibration_const_offset_transform(ref const_offset);

        zedCamera.ResetTrackingWithOffset(tmpHMD.rotation, tmpHMD.position, HmdToZEDCalibration.rotation,
            HmdToZEDCalibration.translation);
    ...
    public void AdjustTrackingAR(Vector3 position, Quaternion orientation, out Quaternion r, out Vector3 t)
    {
        ...
        var hmdTransform = new Pose(InputTracking.GetLocalPosition(XRNode.Head),
        ...
        if (zedReady && latencyCorrectionReady) zedCamera.SetIMUOrientationPrior(ref latencyPose.rotation);
	...
	public void PreRender(Camera cam)
    {
        if (cam == finalLeftEye || cam == finalRightEye)
            if (!zedReady && ZEDManager.IsStereoRig)
            {
                ...
                quadRight.localPosition =
                    InputTracking.GetLocalPosition(XRNode.Head) + quadRight.localRotation * offset;
            }
    }
	...
	public void LoadHmdToZEDCalibration()
    {
        if (hasVRDevice)
        {
            ...
            var specificFolder = Path.Combine(folder, @"Stereolabs\mr");
    ...
    private void CreateDefaultCalibrationFile(string path)
    {
        ...
        {
            var node = "[HMD]";
            var tx = "tx=" + hmdtozedCalibration.translation.x.ToString(CultureInfo.InvariantCulture) +
                     " //Translation x";
            var ty = "ty=" + hmdtozedCalibration.translation.y.ToString(CultureInfo.InvariantCulture) +
                     " //Translation y";
            var tz = "tz=" + hmdtozedCalibration.translation.z.ToString(CultureInfo.InvariantCulture) +
                     " //Translation z";
            var rx = "rx=" + hmdtozedCalibration.rotation.x.ToString(CultureInfo.InvariantCulture) + " //Quaternion x";
            var ry = "ry=" + hmdtozedCalibration.rotation.y.ToString(CultureInfo.InvariantCulture) + " //Quaternion y";
            var rz = "rz=" + hmdtozedCalibration.rotation.z.ToString(CultureInfo.InvariantCulture) + " //Quaternion z";
            var rw = "rw=" + hmdtozedCalibration.rotation.w.ToString(CultureInfo.InvariantCulture) + " //Quaternion w";
    ...
    private bool ParseCalibrationFile(string path)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (splittedLine != null && splittedLine.Length >= 2)
            {
                ...
                if (key == "tx")
                    hmdtozedCalibration.translation.x = float.Parse(field, CultureInfo.InvariantCulture);
                else if (key == "ty")
                    hmdtozedCalibration.translation.y = float.Parse(field, CultureInfo.InvariantCulture);
                else if (key == "tz")
                    hmdtozedCalibration.translation.z = float.Parse(field, CultureInfo.InvariantCulture);
                else if (key == "rx")
                    hmdtozedCalibration.rotation.x = float.Parse(field, CultureInfo.InvariantCulture);
                else if (key == "ry")
                    hmdtozedCalibration.rotation.y = float.Parse(field, CultureInfo.InvariantCulture);
                else if (key == "rz")
                    hmdtozedCalibration.rotation.z = float.Parse(field, CultureInfo.InvariantCulture);
                else if (key == "rw") hmdtozedCalibration.rotation.w = float.Parse(field, CultureInfo.InvariantCulture);
            }
        }


        //Check wrong calibration
        if (hmdtozedCalibration.translation.x == 0.0f && hmdtozedCalibration.translation.y == 0.0f &&
            hmdtozedCalibration.translation.z == 0.0f) CreateDefaultCalibrationFile(path);
