...
public class ZEDControllerTracker : MonoBehaviour
{
    ...
    ///     If S/N is present, then this script will calibrate itself to track the correct device, if that's not the case
    ///     already.
    /// </summary>
...
//Check if the VR headset is connected.
        ...
        if (index == EIndex.None && loadedDevice == "OpenVR")
        {
            if (BIsManufacturerController("HTC") || BIsManufacturerController("Oculus"))
            {
                ...
                //Assigns the Hmd
                if (deviceToTrack == devices.Hmd) index = EIndex.Hmd;
            }
    ...
    [HideInInspector] public EIndex index = EIndex.None;
    ...
    private void OnNewPoses(TrackedDevicePose_t[] poses)
    {
        ...
        //Delaying the saved values inside GetValuePosition() by a factor of latencyCompensation to the millisecond.
        var p = GetValuePosition(1, _latencyCompensation / 1000.0f);
    ...
    public bool BIsManufacturerController(string name)
    {
        ...
        SteamVR.instance.hmd.GetStringTrackedDeviceProperty(0, ETrackedDeviceProperty.Prop_ManufacturerName_String,
            sbType, 1000, ref err);
        return err == ETrackedPropertyError.TrackedProp_Success && sbType.ToString().StartsWith(name);
    }
...
#if UNITY_EDITOR
[CustomEditor(typeof(ZEDControllerTracker))]
[CanEditMultipleObjects]
public class ZEDVRDependencies : Editor
{
    [SerializeField] private static string defineName;
