...
public class ZEDSteamVRControllerManager : MonoBehaviour, ZEDControllerManager
{
	...
	/// </summary>
	[HideInInspector] public Dictionary<int, DelayedPad> delayedPads = new Dictionary<int, DelayedPad>();
	...
	/// </summary>
	[HideInInspector] public List<SteamVR_TrackedController> controllers = new List<SteamVR_TrackedController>();

	/// <summary>
	///     List of the gameObjects controlled by SteamVR
	/// </summary>
	[HideInInspector] public List<GameObject> controllersGameObject = new List<GameObject>();

	/// <summary>
	///     List of the devices controlled by SteamVR
	/// </summary>
	[HideInInspector] public List<SteamVR_Controller.Device> devices = new List<SteamVR_Controller.Device>();
	...
	/// </summary>
	private readonly Dictionary<int, List<TimedPoseData>> poseData = new Dictionary<int, List<TimedPoseData>>();

	/// <summary>
	///     Current controller ID which is holding the ZED
	/// </summary>
	[HideInInspector] [Tooltip("Controller ID holding the ZED")]
    public int controllerIndexZEDHolder = -1;
	...
	/// </summary>
	[Range(0, 200)] public int latencyCompensation = 78;

	/// <summary>
	///     Timers between each checks, to registers new pads
	/// </summary>
	private float timerVive;

    private readonly float timerMaxVive = 1.0f;

	/// <summary>
	///     Returns the current ID of the controller holding the ZED
	/// </summary>
	public int ControllerIndexZEDHolder => controllerIndexZEDHolder;
	...
	/// </summary>
	[HideInInspector] public bool padsSet;
	...
	/// </summary>
	public GameObject modelToReplaceVivePads;

	/// <summary>
	///     Checks if the holder of the ZED is ready
	/// </summary>
	private bool ZEDControllerSet;
	...
	public void LoadIndex(string path)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (splittedLine.Length >= 2)
            {
                ...
                if (key == "indexController") SNHolder = field;
            }
        }
    }
    ...
    {
        foreach (var pad in delayedPads)
        {
            var i = pad.Key;
            ...
            var p = GetValuePosition(i, latencyCompensation / 1000.0f);
    ...
    private void RegisterExistingPads()
    {
        if (cameraRig != null) controllerManager = cameraRig.transform.GetComponent<SteamVR_ControllerManager>();
        ...
        foreach (var o in controllerManager.objects)
    ...
    {
        if (newPoses != null) newPoses.enabled = false;
    }

    public bool GetPadIndex()
    {
        foreach (var c in controllersGameObject)
    ...
    {
        poseData.Clear();
        foreach (var o in delayedPads.Values) Destroy(o.o);
        delayedPads.Clear();
        ...
        foreach (var d in devices)
        {
            if (poseData.ContainsKey((int) d.index))
                continue;
            var p = new DelayedPad();
            if (modelToReplaceVivePads == null)
            {
                p.o = new GameObject();
                p.o.name = "VirtualDelayedPad_" + (int) d.index;
                p.o.layer = ZEDCamera.Tag;
                var m = p.o.AddComponent<SteamVR_RenderModel>();
    ...
    {
        var tracked = c.GetComponent<SteamVR_TrackedObject>();
        return tracked != null && (int) tracked.index != -1 && (int) tracked.index != 0 &&
               !devices.Contains(SteamVR_Controller.Input((int) tracked.index));
    }
    ...
    public bool SetPads(bool noCameraTracking)
    {
        ...
        {
            foreach (var d in devices)
                if (d.index != OpenVR.k_unTrackedDeviceIndexInvalid && d.index != leftIndex && d.index != rightIndex &&
                    leftIndex != OpenVR.k_unTrackedDeviceIndexInvalid &&
                    rightIndex != OpenVR.k_unTrackedDeviceIndexInvalid
                    || d.GetPressDown(EVRButtonId.k_EButton_SteamVR_Trigger))
                {
        ...
        foreach (var d in devices)
        {
            ...
            {
                while (index < controllersGameObject.Count && !controllersGameObject[index].activeInHierarchy) index++;
                var c = controllersGameObject[index].GetComponent<SteamVR_TrackedController>();
                if (c == null) c = controllersGameObject[index].AddComponent<SteamVR_TrackedController>();
                if (controllerObject == null) controllerObject = controllersGameObject[index];
    ...
    {
        var zedCamera = transform.GetChild(0).GetComponent<Camera>();
    ...
    private Pose GetValuePosition(int index, float timeDelay)
    {
        ...
        for (var i = 0; i < poseData[index].Count; ++i)
            if (poseData[index][i].timestamp > idealTS)
            {
                var currentIndex = i;
                if (currentIndex > 0)
                {
                    var timeBetween = poseData[index][currentIndex].timestamp -
                                      poseData[index][currentIndex - 1].timestamp;
                    var alpha = (Time.time - poseData[index][currentIndex - 1].timestamp - timeDelay) / timeBetween;

                    var pos = Vector3.Lerp(poseData[index][currentIndex - 1].position,
                        poseData[index][currentIndex].position, alpha);
                    var rot = Quaternion.Lerp(poseData[index][currentIndex - 1].rotation,
                        poseData[index][currentIndex].rotation, alpha);
    ...
    {
        var currentPoseData = new TimedPoseData();
    ...
    private void Update()
    {
        ...
        {
            timerVive = 0;
            if (!padsSet) GetPadIndex();

            if (devices.Count != delayedPads.Count) CreateNewPads();
            ...
            if (controllerIndexZEDHolder == -1)
            {
                ...
                {
                    foreach (var d in devices)
                        if (SteamVR.instance.GetStringProperty(ETrackedDeviceProperty.Prop_SerialNumber_String, d.index)
                            .Equals(SNHolder))
                        {
                            controllerIndexZEDHolder = (int) d.index;
                            padsAreInit = true;
            ...
            if (previousStateIndex != controllerIndexZEDHolder && controllerIndexZEDHolder != -1)
            {
                ...
                if (ZEDOnPadIndexSet != null) ZEDOnPadIndexSet();
            }
        }
    }
	...
	/// <returns></returns>
	private EVRButtonId ConvertToButton(CONTROLS_BUTTON button)
    {
        switch (button)
        {
            case CONTROLS_BUTTON.ONE:
                return model == HMD.OCULUS ? EVRButtonId.k_EButton_A : EVRButtonId.k_EButton_ApplicationMenu;
            case CONTROLS_BUTTON.THREE: return EVRButtonId.k_EButton_ApplicationMenu;
            case CONTROLS_BUTTON.PRIMARY_THUBMSTICK: return EVRButtonId.k_EButton_SteamVR_Touchpad;
            case CONTROLS_BUTTON.SECONDARY_THUMBSTICK: return EVRButtonId.k_EButton_SteamVR_Touchpad;
        }
	...
	/// <param name="button"></param>
	/// <returns></returns>
	private EVRButtonId ConvertToAxis(CONTROLS_AXIS1D button)
    {
        switch (button)
        {
            case CONTROLS_AXIS1D.PRIMARY_INDEX_TRIGGER: return EVRButtonId.k_EButton_SteamVR_Trigger;
            case CONTROLS_AXIS1D.SECONDARY_INDEX_TRIGGER: return EVRButtonId.k_EButton_SteamVR_Trigger;
            case CONTROLS_AXIS1D.PRIMARY_HAND_TRIGGER: return EVRButtonId.k_EButton_Grip;
            case CONTROLS_AXIS1D.SECONDARY_HAND_TRIGGER: return EVRButtonId.k_EButton_Grip;
        }
	...
	/// <returns></returns>
	public bool GetDown(CONTROLS_BUTTON idButton, int idPad)
    {
#if ZED_STEAM_VR
	...
	public float GetHairTrigger(CONTROLS_AXIS1D idTrigger, int idPad)
    {
        ...
        return SteamVR_Controller.Input(idPad).GetAxis((EVRButtonId) idTrigger).magnitude;
#else
		return 0;
		#endif
    }
...
#if UNITY_EDITOR
[CustomEditor(typeof(ZEDSteamVRControllerManager))]
[CanEditMultipleObjects]
public class ZEDSteamVRDependencies : Editor
{
    [SerializeField] private const string defineName = "ZED_STEAM_VR";

    private const string packageName = "SteamVR";
