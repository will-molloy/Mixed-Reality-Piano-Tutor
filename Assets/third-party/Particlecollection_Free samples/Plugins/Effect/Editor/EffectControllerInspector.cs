...
public class EffectControllerInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        var effectCtrl = target as EffectController;
        ...
        {
            if (nCnt >= effectCtrl.m_kEffectGenList.Count) effectCtrl.m_kEffectGenList.Add(new EffectData());

            var effectData = effectCtrl.m_kEffectGenList[nCnt];
            if (effectData == null)
                continue;
            if (bShowAll)
            ...
            effectData.m_bFoldoutOpen =
                EditorGUILayout.Foldout(effectData.m_bFoldoutOpen, "Effect " + nCnt + " Setting");
            if (effectData.m_bFoldoutOpen)
            {
                effectData.m_fTimeSec = EditorGUILayout.FloatField("Shot Time", effectData.m_fTimeSec);
                effectData.m_goEffect =
                    EditorGUILayout.ObjectField("Obj", effectData.m_goEffect, typeof(GameObject), true) as GameObject;
                ...
                if (effectData.m_bTransformFoldout)
                {
                    ...
                    GUILayout.EndHorizontal();
                    if (EditorGUI.EndChangeCheck()) effectCtrl.UpdateEffectTransformByIndex(nCnt);
                    EditorGUI.indentLevel--;
                }

                var particleSystem = effectCtrl.CheckHasParticleSystem(nCnt);
                var renderEffect = effectCtrl.CheckHasRenderEffectScript(nCnt);
                if (particleSystem == null)
                {
                    effectData.m_bSortingFoldout =
                        EditorGUILayout.Foldout(effectData.m_bSortingFoldout, "Sorting Layer");
                    /// Sorting panel.
                    if (effectData.m_bSortingFoldout)
                    {
                        ...
                        effectData.m_SortingLayerID =
                            EditorGUILayout.IntPopup(effectData.m_SortingLayerID, m_LayerName, m_LayerID);
                        EditorGUILayout.EndHorizontal();
        ...
        for (; nCnt < effectCtrl.m_kEffectGenList.Count; nCnt++) effectCtrl.m_kEffectGenList.RemoveAt(nCnt);
