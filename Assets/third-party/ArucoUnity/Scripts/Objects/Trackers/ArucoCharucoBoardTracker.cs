    ...
    public class ArucoCharucoBoardTracker : ArucoObjectTracker
    {
        ...
        public override void Detect(int cameraId, Aruco.Dictionary dictionary, Cv.Mat image)
        {
            ...
            foreach (var arucoCharucoBoard in arucoTracker.GetArucoObjects<ArucoCharucoBoard>(dictionary))
            {
                ...
                {
                    Aruco.RefineDetectedMarkers(image, arucoCharucoBoard.Board,
            ...
            foreach (var arucoCharucoBoard in arucoTracker.GetArucoObjects<ArucoCharucoBoard>(dictionary))
                if (arucoCharucoBoard.DetectedIds.Size() > 0)
                {
                    if (arucoTracker.DrawDetectedCharucoMarkers)
                        Aruco.DrawDetectedCornersCharuco(image, arucoCharucoBoard.DetectedCorners,
                            arucoCharucoBoard.DetectedIds);
        ...
        public override void EstimateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            foreach (var arucoCharucoBoard in arucoTracker.GetArucoObjects<ArucoCharucoBoard>(dictionary))
            {
                ...
                var validTransform = false;

                if (arucoTracker.MarkerTracker.DetectedMarkers[cameraId][dictionary] > 0 &&
                    arucoCameraUndistortion != null)
                    validTransform = Aruco.EstimatePoseCharucoBoard(arucoCharucoBoard.DetectedCorners,
                        arucoCharucoBoard.DetectedIds,
                        (Aruco.CharucoBoard) arucoCharucoBoard.Board,
                        arucoCameraUndistortion.RectifiedCameraMatrices[cameraId],
                        arucoCameraUndistortion.UndistortedDistCoeffs[cameraId],
                        out rvec, out tvec);
        ...
        public override void UpdateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            foreach (var arucoCharucoBoard in arucoTracker.GetArucoObjects<ArucoCharucoBoard>(dictionary))
                if (arucoCharucoBoard.Rvec != null)
                    arucoCameraDisplay.PlaceArucoObject(arucoCharucoBoard.transform, cameraId,
                        arucoCharucoBoard.Tvec.ToPosition(),
