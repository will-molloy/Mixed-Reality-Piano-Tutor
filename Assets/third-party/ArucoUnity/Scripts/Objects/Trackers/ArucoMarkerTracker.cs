...
namespace ArucoUnity.Objects.Trackers
{
    public class ArucoMarkerTracker : ArucoObjectTracker
    {
      ...
      /// <summary>
      ///     Vector of identifiers of the detected markers on each <see cref="ArucoCamera.Images" />. Updated by
      ///     <see cref="Detect" />.
      /// </summary>
      public Dictionary<Aruco.Dictionary, Std.VectorInt>[] MarkerIds { get; protected internal set; }

      /// <summary>
      ///     Vector of the corners with not a correct identification on each <see cref="ArucoCamera.Images" />. Updated by
      ///     <see cref="Detect" />.
      /// </summary>
      public Dictionary<Aruco.Dictionary, Std.VectorVectorPoint2f>[] RejectedCandidateCorners
        {
            get;
      ...
      public Dictionary<Aruco.Dictionary, Std.VectorVec3d>[] MarkerTvecs { get; protected internal set; }
        ...
        public override void Activate(IArucoObjectsTracker arucoTracker)
        {
            ...
            RejectedCandidateCorners =
                new Dictionary<Aruco.Dictionary, Std.VectorVectorPoint2f>[arucoCamera.CameraNumber];
            ...
            for (var cameraId = 0; cameraId < arucoCamera.CameraNumber; cameraId++)
            {
                ...
                {
                    var dictionary = arucoObjectDictionary.Key;
        ...
        public override void Detect(int cameraId, Aruco.Dictionary dictionary, Cv.Mat image)
        {
            ...
            Aruco.DetectMarkers(image, dictionary, out markerCorners, out markerIds, arucoTracker.DetectorParameters,
        ...
        public override void Draw(int cameraId, Aruco.Dictionary dictionary, Cv.Mat image)
        {
            ...
            if (DetectedMarkers[cameraId][dictionary] > 0)
            {
                ...
                if (arucoTracker.DrawDetectedMarkers)
                    Aruco.DrawDetectedMarkers(image, MarkerCorners[cameraId][dictionary],
                        MarkerIds[cameraId][dictionary]);

                // Draw axes of detected tracked markers
                if (arucoTracker.DrawAxes && arucoCameraUndistortion != null &&
                    MarkerRvecs[cameraId][dictionary] != null)
                    for (uint i = 0; i < DetectedMarkers[cameraId][dictionary]; i++)
                    {
                        ArucoObject foundArucoObject;
                        var detectedMarkerHashCode =
                            ArucoMarker.GetArucoHashCode(MarkerIds[cameraId][dictionary].At(i));
                        if (arucoTracker.ArucoObjects[dictionary]
                            .TryGetValue(detectedMarkerHashCode, out foundArucoObject))
                            Aruco.DrawAxis(image, arucoCameraUndistortion.RectifiedCameraMatrices[cameraId],
                                arucoCameraUndistortion.UndistortedDistCoeffs[cameraId],
                                MarkerRvecs[cameraId][dictionary].At(i), MarkerTvecs[cameraId][dictionary].At(i),
        ...
        public override void EstimateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            if (DetectedMarkers[cameraId][dictionary] > 0 && arucoCameraUndistortion != null)
                Aruco.EstimatePoseSingleMarkers(MarkerCorners[cameraId][dictionary], estimatePoseMarkerLength,
                    arucoCameraUndistortion.RectifiedCameraMatrices[cameraId],
        ...
        public override void UpdateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            if (MarkerRvecs[cameraId][dictionary] != null)
                ...
                {
                    ArucoObject foundArucoObject;
                    var detectedMarkerHashCode = ArucoMarker.GetArucoHashCode(MarkerIds[cameraId][dictionary].At(i));
                    if (arucoTracker.ArucoObjects[dictionary].TryGetValue(detectedMarkerHashCode, out foundArucoObject))
                    {
                        var localPosition = MarkerTvecs[cameraId][dictionary].At(i).ToPosition() *
                                            foundArucoObject.MarkerSideLength / estimatePoseMarkerLength;
