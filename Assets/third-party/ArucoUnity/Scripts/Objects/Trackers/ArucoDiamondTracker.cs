...
namespace ArucoUnity.Objects.Trackers
{
    public class ArucoDiamondTracker : ArucoObjectTracker
    {
        ...
        public override void Detect(int cameraId, Aruco.Dictionary dictionary, Cv.Mat image)
        {
            ...
            var markerTracker = arucoTracker.MarkerTracker;
            ...
            if (markerTracker.DetectedMarkers[cameraId][dictionary] > 0)
            {
                if (arucoCameraUndistortion == null)
                    Aruco.DetectCharucoDiamond(image, markerTracker.MarkerCorners[cameraId][dictionary],
                        markerTracker.MarkerIds[cameraId][dictionary],
        ...
        public override void Draw(int cameraId, Aruco.Dictionary dictionary, Cv.Mat image)
        {
            ...
            if (DetectedDiamonds[cameraId][dictionary] > 0)
            {
                ...
                if (arucoTracker.DrawDetectedDiamonds)
                    Aruco.DrawDetectedDiamonds(image, DiamondCorners[cameraId][dictionary],
                        DiamondIds[cameraId][dictionary]);

                // Draw axes of detected diamonds
                if (arucoTracker.DrawAxes && arucoCameraUndistortion != null &&
                    DiamondRvecs[cameraId][dictionary] != null)
                    for (uint i = 0; i < DetectedDiamonds[cameraId][dictionary]; i++)
                        Aruco.DrawAxis(image, arucoCameraUndistortion.RectifiedCameraMatrices[cameraId],
                            arucoCameraUndistortion.UndistortedDistCoeffs[cameraId],
                            DiamondRvecs[cameraId][dictionary].At(i), DiamondTvecs[cameraId][dictionary].At(i),
                            DrawAxisLength);
            }
        }
        ...
        public override void EstimateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            if (DetectedDiamonds[cameraId][dictionary] > 0 && arucoCameraUndistortion != null)
                Aruco.EstimatePoseSingleMarkers(DiamondCorners[cameraId][dictionary], EstimatePoseSquareLength,
                    arucoCameraUndistortion.RectifiedCameraMatrices[cameraId],
        ...
        public override void UpdateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            if (DiamondRvecs[cameraId][dictionary] != null)
                for (uint i = 0; i < DetectedDiamonds[cameraId][dictionary]; i++)
                {
                    ...
                    {
                        var positionFactor = foundArucoDiamond.SquareSideLength * EstimatePoseSquareLength /
                                             DetectSquareMarkerLengthRate; // Equal to marker length
                        arucoCameraDisplay.PlaceArucoObject(foundArucoDiamond.transform, cameraId,
                            arucoTracker.MarkerTracker.MarkerTvecs[cameraId][dictionary].At(i).ToPosition() *
                            positionFactor,
        ...
        protected virtual bool TryGetArucoDiamond(int cameraId, Aruco.Dictionary dictionary, uint arucoObjectId,
            out ArucoDiamond arucoDiamond)
        {
            var detectedDiamondIds = new int[4];
            for (var j = 0; j < 4; j++)
                detectedDiamondIds[j] = DiamondIds[cameraId][dictionary].At(arucoObjectId).Get(j);

            ArucoObject foundArucoObject;
            var detectedDiamondHashCode = ArucoDiamond.GetArucoHashCode(detectedDiamondIds);
            if (arucoTracker.ArucoObjects[dictionary].TryGetValue(detectedDiamondHashCode, out foundArucoObject))
            {
                ...
                if (arucoDiamond != null) return true;
            }
