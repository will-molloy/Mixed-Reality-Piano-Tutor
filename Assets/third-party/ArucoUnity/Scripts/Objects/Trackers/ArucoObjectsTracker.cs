...
{
  /// <summary>
  ///     Detects <see cref="ArucoObject" />, displays detections and applies the estimated transforms to gameObjects
  ///     associated to the ArUco objects.
  ///     See the OpenCV documentation for more information about the marker detection:
  ...
  public class ArucoObjectsTracker : ArucoObjectsController, IArucoObjectsTracker
    {
        ...
        [SerializeField] [Tooltip("The display associated with the camera.")]
        private ArucoCameraDisplay arucoCameraDisplay;

        [SerializeField] [Tooltip("Display the axis of the detected boards and diamonds.")]
        private bool drawAxes = true;

        [SerializeField] [Tooltip("Display the markers of the detected ChArUco boards.")]
        private bool drawDetectedCharucoMarkers = true;

        [SerializeField] [Tooltip("Display the detected diamonds.")]
        private bool drawDetectedDiamonds = true;

        [SerializeField] [Tooltip("Display the detected markers in the CameraImageTexture.")]
        private bool drawDetectedMarkers = true;

        [SerializeField] [Tooltip("Display the rejected markers candidates.")]
        private bool drawRejectedCandidates;
        ...
        public bool RefineDetectedMarkers
        {
            get { return refineDetectedMarkers; }
            set { refineDetectedMarkers = value; }
        }

        public bool DrawDetectedMarkers
        {
            get { return drawDetectedMarkers; }
            set { drawDetectedMarkers = value; }
        }

        public bool DrawRejectedCandidates
        {
            get { return drawRejectedCandidates; }
            set { drawRejectedCandidates = value; }
        }
        ...
        public bool DrawDetectedCharucoMarkers
        {
            get { return drawDetectedCharucoMarkers; }
            set { drawDetectedCharucoMarkers = value; }
        }

        public bool DrawDetectedDiamonds
        {
            get { return drawDetectedDiamonds; }
            set { drawDetectedDiamonds = value; }
        }
        ...
        {
            if (!IsConfigured) throw new Exception("The tracker must be configured before tracking ArUco objects.");
      ...
      protected override void Awake()
        {
            ...
            if (arucoCameraDisplay != null) ArucoCameraDisplay = arucoCameraDisplay;
        }
      ...
      protected override void Configuring()
        {
            if (ArucoCameraDisplay == null)
                throw new ArgumentNullException("ArucoCameraDisplay",
                    "This property needs to be set for the configuration.");
      ...
      /// <summary>
      ///     Initializes the tracking, activates the trackers, susbcribes to the
      ///     <see cref="ArucoObjectsController{T}.ArucoObjectAdded" /> and
      ...
      protected override void Starting()
        {
            ...
            {
                CopyBackImages = DrawDetectedMarkers || DrawRejectedCandidates || DrawAxes ||
                                 DrawDetectedCharucoMarkers || DrawDetectedDiamonds
            };
            trackingThread.Start();
        }

      /// <summary>
      ///     Unsuscribes from ArucoObjectController events, deactivates the trackers and abort the tracking thread and stops the
      ///     tracking thread.
      /// </summary>
      ...
      protected virtual void ArucoObjectsController_ArucoObjectAdded(ArucoObject arucoObject)
        {
            if (arucoObject.GetType() != typeof(ArucoMarker))
            {
                ...
                if (!additionalTrackers.TryGetValue(arucoObject.GetType(), out tracker))
                    throw new ArgumentException("No tracker found for the type '" + arucoObject.GetType() + "'.",
                        "arucoObject");
                if (!tracker.IsActivated) tracker.Activate(this);
            }
        }
      ...
      protected virtual void ArucoObjectsController_ArucoObjectRemoved(ArucoObject arucoObject)
        {
            ArucoObjectTracker tracker = null;
            if (arucoObject.GetType() == typeof(ArucoMarker) ||
                !additionalTrackers.TryGetValue(arucoObject.GetType(), out tracker)) return;
            ...
            if (tracker.IsActivated)
            {
                ...
                foreach (var arucoObjectDictionary in ArucoObjects)
                {
                    ...
                    if (!deactivateTracker) break;
                }

                if (deactivateTracker) tracker.Deactivate();
            }
        }
      ...
      protected void ArucoCamera_ImagesUpdated()
        {
            try
            {
                ...
                if (trackingThread.ImagesUpdated)
                {
                    ...
                    if (ArucoCameraDisplay != null) UpdateTransforms();
                }
            }
            catch (Exception e)
            {
      ...
      protected void TrackArucoObjects(Cv.Mat[] images)
        {
            Detect(images);
            EstimateTransforms();
            if (trackingThread.CopyBackImages) Draw(images);
        }
