    ...
    public class ArucoGridBoardTracker : ArucoObjectTracker
    {
        ...
        public override void Detect(int cameraId, Aruco.Dictionary dictionary, Cv.Mat image)
        {
            ...
            if (arucoTracker.RefineDetectedMarkers &&
                arucoTracker.MarkerTracker.DetectedMarkers[cameraId][dictionary] > 0)
                foreach (var arucoBoard in arucoTracker.GetArucoObjects<ArucoGridBoard>(dictionary))
                {
                    Aruco.RefineDetectedMarkers(image, arucoBoard.Board,
                        markerTracker.MarkerCorners[cameraId][dictionary],
                        markerTracker.MarkerIds[cameraId][dictionary],
                        markerTracker.RejectedCandidateCorners[cameraId][dictionary]);
                    markerTracker.DetectedMarkers[cameraId][dictionary] =
                        (int) markerTracker.MarkerIds[cameraId][dictionary].Size();
                }
        }
        ...
        public override void EstimateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            foreach (var arucoGridBoard in arucoTracker.GetArucoObjects<ArucoGridBoard>(dictionary))
            {
                ...
                var markersUsedForEstimation = 0;

                if (arucoTracker.MarkerTracker.DetectedMarkers[cameraId][dictionary] > 0 &&
                    arucoCameraUndistortion != null)
                    markersUsedForEstimation = Aruco.EstimatePoseBoard(
                        arucoTracker.MarkerTracker.MarkerCorners[cameraId][dictionary],
                        arucoTracker.MarkerTracker.MarkerIds[cameraId][dictionary], arucoGridBoard.Board,
        ...
        public override void UpdateTransforms(int cameraId, Aruco.Dictionary dictionary)
        {
            ...
            foreach (var arucoGridBoard in arucoTracker.GetArucoObjects<ArucoGridBoard>(dictionary))
                if (arucoGridBoard.Rvec != null)
                {
                    ...
                    arucoCameraDisplay.PlaceArucoObject(arucoGridBoard.transform, cameraId, position,
                        arucoGridBoard.Rvec.ToRotation());
                }
        }
    }
}