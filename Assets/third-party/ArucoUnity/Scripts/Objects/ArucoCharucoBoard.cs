  ...
  public class ArucoCharucoBoard : ArucoBoard
    {
        ...
        [SerializeField] [Tooltip("Number of squares in the X direction.")]
        private int squaresNumberX;

        [SerializeField] [Tooltip("Number of squares in the Y direction.")]
      ...
      public bool ValidTransform { get; internal set; }
        ...
        public override Vector3 GetGameObjectScale()
        {
            ImageSize = new Vector2(
                SquaresNumberX * SquareSideLength + 2 * MarginsLength,
                SquaresNumberY * SquareSideLength + 2 * MarginsLength
            );
        ...
        public override Cv.Mat Draw()
        {
            ...
            if (!EditorApplication.isPlayingOrWillChangePlaymode &&
                (SquaresNumberX <= 1 || SquaresNumberY <= 1 || SquareSideLength <= 0
                 || MarkerSideLength <= 0 || SquareSideLength <= MarkerSideLength || MarkerBorderBits <= 0 ||
                 Dictionary == null))
                return null;
#endif
            var squareSideLength = GetInPixels(SquareSideLength);
            var markerSideLength = GetInPixels(MarkerSideLength);
            var board = Aruco.CharucoBoard.Create(SquaresNumberX, SquaresNumberY, squareSideLength, markerSideLength,
                Dictionary);
        ...
        {
            return "ArUcoUnity_ChArUcoBoard_" + Dictionary.Name + "_X_" + SquaresNumberX + "_Y_" + SquaresNumberY +
                   "_SquareSize_" + SquareSideLength
        ...
        protected override void UpdateBoard()
        {
            ...
            Board = Aruco.CharucoBoard.Create(SquaresNumberX, SquaresNumberY, SquareSideLength, MarkerSideLength,
                Dictionary);
        }
      ...
      public static int GetArucoHashCode(int squaresNumberX, int squaresNumberY, float markerSideLength,
            float squareSideLength)
        {
            ...
            hashCode = hashCode * 31 +
                       Mathf.RoundToInt(markerSideLength * 1000); // MarkerSideLength is not less than millimetres
            hashCode = hashCode * 31 +
                       Mathf.RoundToInt(markerSideLength * 1000); // SquareSideLength is not less than millimetres
            return hashCode;
        }
    }
}