  ...
  public abstract class ConfigurableController : MonoBehaviour, IConfigurableController
    {
        ...
        private readonly HashSet<IConfigurableController> dependencies = new HashSet<IConfigurableController>();
        private readonly HashSet<IConfigurableController> stoppedDependencies = new HashSet<IConfigurableController>();
        ...
        {
            if (IsStarted) throw new Exception("Stop the controller before updating the dependencies.");

            dependencies.Add(dependency);
            if (!dependency.IsStarted) stoppedDependencies.Add(dependency);
        ...
        {
            if (IsStarted) throw new Exception("Stop the controller before configure it.");
        ...
        {
            if (!IsConfigured || !IsStarted) throw new Exception("Configure and start the controller before stop it.");
      ...
      protected virtual void OnConfigured()
        {
            ...
            if (stoppedDependencies.Count == 0) OnReady();
        }
      ...
      private void SetAutoStart(bool value)
        {
            autoStart = value;
            if (AutoStart) ConfigureController();
        }
      ...
      private void Dependency_Started(IConfigurableController dependency)
        {
            stoppedDependencies.Remove(dependency);
            if (IsConfigured && stoppedDependencies.Count == 0) OnReady();
        }
