  ...
  public class WebcamController : MonoBehaviour
    {
      ...
      public Action<WebcamController> Started = delegate { };
        ...
        private readonly List<Texture2D> textures2D = new List<Texture2D>();
      ...
      public List<Texture2D> Textures2D
        {
            get
            {
                for (var cameraId = 0; cameraId < Textures.Count; cameraId++)
      ...
      /// </summary>
      public TextureFormat Textures2DFormat { get; set; } = TextureFormat.RGB24;
      ...
      public void StopWebcams()
        {
            if (!IsConfigured || !IsStarted) throw new Exception("Configure the controller and start the controller.");

            IsStarted = false;
            if (starting) StopCoroutine(StartingAsync());

            foreach (var webcam in Textures) webcam.Stop();
        }
      ...
      protected IEnumerator StartingAsync()
        {
            starting = true;
            foreach (var webcam in Textures) webcam.Play();

            bool webcamsStarted;
            do
            {
                webcamsStarted = true;
                foreach (var texture in Textures) webcamsStarted &= texture.width > 100;
