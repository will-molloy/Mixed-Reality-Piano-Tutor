  ...
  public abstract class OmnidirCameraUndistortionGeneric<T> : ArucoCameraUndistortionGeneric<T>
        where T : ArucoCamera
    {
      ...
      public enum RectificationTypes
        ...
        [SerializeField] [Tooltip("The algorithm to use for the recitification of the images.")]
        private RectificationTypes rectificationType = RectificationTypes.Perspective;

        // Variables

        protected Dictionary<RectificationTypes, Cv.Omnidir.Rectifify> rectifyFlags =
            new Dictionary<RectificationTypes, Cv.Omnidir.Rectifify>
            {
      ...
      /// </summary>
      public RectificationTypes RectificationType
        {
            get { return rectificationType; }
            set { rectificationType = value; }
        }

      /// <summary>
      ///     Gets or sets the desired field of view for the Unity cameras shooting the undistorted and rectified images.
      /// </summary>
      public float[] PerspectiveFieldOfViews
        {
            get { return perspectiveFieldOfViews; }
            set { perspectiveFieldOfViews = value; }
        }

        // MonoBehaviour methods

      /// <summary>
      ///     Resizes the length of the <see cref="perspectiveFieldOfViews" /> editor field to
      ///     <see cref="ArucoCamera.CameraNumber" /> if different.
      /// </summary>
      protected virtual void OnValidate()
        {
            if (ArucoCamera != null && perspectiveFieldOfViews != null &&
                perspectiveFieldOfViews.Length != ArucoCamera.CameraNumber)
      ...
      /// <summary>
      ///     Throw exception if <see cref="PerspectiveFieldOfViews" /> length is different than
      ...
      /// <summary>
      ///     Initializes the <see cref="RectifiedCameraMatrices" /> using the <see cref="PerspectiveFieldOfViews" /> values for
      ///     perspective rectification
      ///     or uses the recommended values: https://docs.opencv.org/3.3.1/dd/d12/tutorial_omnidir_calib_main.html. Initializes
      ///     the
      ///     <see cref="RectificationMatrices" /> to identity matrix.
      /// </summary>
      protected override void InitializeRectification()
        {
            for (var cameraId = 0; cameraId < CameraParameters.CameraNumber; cameraId++)
            {
                ...
                {
                    var cameraFocalLength = imageHeight /
                                            (2f * Mathf.Tan(0.5f * PerspectiveFieldOfViews[cameraId] * Mathf.Deg2Rad));
        ...
        {
            for (var cameraId = 0; cameraId < CameraParameters.CameraNumber; cameraId++)
                Cv.Omnidir.InitUndistortRectifyMap(CameraParameters.CameraMatrices[cameraId],
                    CameraParameters.DistCoeffs[cameraId],
                    CameraParameters.OmnidirXis[cameraId], RectificationMatrices[cameraId],
                    RectifiedCameraMatrices[cameraId],
