  ...
  public class StereoVRArucoCameraDisplay : ArucoCameraDisplayGeneric<StereoArucoCamera, ArucoCameraUndistortion>
    {
        ...
        [SerializeField] [Tooltip("The background displaying the image of the left physical camera in ArucoCamera.")]
        private Renderer leftBackground;

        [SerializeField] [Tooltip("The Unity virtual camera that will shoot the left eye background.")]
        private Camera leftBackgroundCamera;

        [SerializeField] [Tooltip("The background displaying the image of the right physical camera in ArucoCamera.")]
        private Renderer rightBackground;

        [SerializeField] [Tooltip("The Unity virtual camera that will shoot the right eye background.")]
      ...
      /// <summary>
      ///     Populates <see cref="Eyes" />, <see cref="ArucoCameraDisplayGeneric.Cameras" />,
      ///     <see cref="ArucoCameraDisplayGeneric.BackgroundCameras" />
      ...
      protected override void Awake()
        ...
        public override void PlaceArucoObject(Transform arucoObject, int cameraId, Vector3 localPosition,
            Quaternion localRotation)
        {
            ...
            arucoObject.transform.localPosition = new Vector3(localPosition.x, localPosition.y,
                                                      arucoObjectPlacementZFactor * localPosition.z)
      ...
      /// <summary>
      ///     Places the <see cref="ArucoCameraDisplayGeneric.Backgrounds" /> taking account of the difference of the focal
      ///     length between the VR
      ///     <see cref="ArucoCameraDisplayGeneric.Cameras" /> and from
      ///     <see cref="ArucoCameraUndistortion.RectificationMatrices" />.
      /// </summary>
      ...
      protected override void ConfigureRectifiedBackground(int cameraId)
        {
            ...
            cameraFocalLength = Cameras[cameraId].pixelHeight /
                                (2f * Mathf.Tan(0.5f * Cameras[cameraId].fieldOfView * Mathf.Deg2Rad));
            arucoObjectPlacementZFactor = cameraFocalLength / focalLength.y;

            var localPositionX = (0.5f * imageWidth - principalPoint.x) / cameraFocalLength * cameraBackgroundDistance;
            var localPositionY =
                -(0.5f * imageHeight - principalPoint.y) / cameraFocalLength * cameraBackgroundDistance;

            var localScaleX = imageWidth / cameraFocalLength * cameraBackgroundDistance;
            var localScaleY = imageHeight / cameraFocalLength * cameraBackgroundDistance;
