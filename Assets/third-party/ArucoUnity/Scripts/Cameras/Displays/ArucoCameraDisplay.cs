  ...
  public abstract class ArucoCameraDisplay : ArucoCameraController, IArucoCameraDisplay
    {
      ...
      public IArucoCameraUndistortion ArucoCameraUndistortion { get; set; }
        ...
        public virtual void PlaceArucoObject(Transform arucoObject, int cameraId, Vector3 localPosition,
      ...
      protected override void Configuring()
        {
            base.Configuring();
            if (ArucoCameraUndistortion != null) AddDependency(ArucoCameraUndistortion);
        }
      ...
      /// <summary>
      ///     Activates or deactivates the <see cref="Cameras" />, the <see cref="BackgroundCameras" /> and the
      ///     <see cref="Backgrounds" />.
      /// </summary>
      ...
      /// <summary>
      ///     Places the <see cref="Backgrounds" /> in front of the corresponding <see cref="BackgroundCameras" /> centered and
      ...
      protected virtual void ConfigureDefaultBackgrounds()
        {
            for (var cameraId = 0; cameraId < ArucoCamera.CameraNumber; cameraId++)
            {
                ...
                {
                    localScale.x = 2f * cameraBackgroundDistance * BackgroundCameras[cameraId].aspect *
                                   Mathf.Tan(0.5f * BackgroundCameras[cameraId].fieldOfView * Mathf.Deg2Rad);
                    localScale.y = localScale.x / ArucoCamera.ImageRatios[cameraId];
                }
                else
                {
                    localScale.y = 2f * cameraBackgroundDistance *
                                   Mathf.Tan(0.5f * BackgroundCameras[cameraId].fieldOfView * Mathf.Deg2Rad);
      ...
      /// <summary>
      ///     Configures the field of view of a <see cref="Cameras" /> according to the vertical focal length of the
      ///     corresponding rectified camera matrix
      ///     in <see cref="ArucoCameraUndistortion.RectifiedCameraMatrices" />. If the camera targets an eye in VR mode, Unity
      ...
      protected virtual void ConfigureRectifiedCamera(int cameraId)
        {
            ...
            var cameraF = ArucoCameraUndistortion.RectifiedCameraMatrices[cameraId].GetCameraFocalLengths();

            var fovY = 2f * Mathf.Atan(0.5f * imageHeight / cameraF.y) * Mathf.Rad2Deg;
            Cameras[cameraId].fieldOfView = fovY;
            BackgroundCameras[cameraId].fieldOfView = fovY;
        }

      /// <summary>
      ///     Places a <see cref="Backgrounds" /> in front of the corresponding <see cref="BackgroundCameras" /> centered with
      ///     the principal point of the
      ///     corresponding rectified camera matrix in <see cref="ArucoCameraUndistortion.RectifiedCameraMatrices" /> and scaled
      ///     to fit in the field of
      ...
      protected virtual void ConfigureRectifiedBackground(int cameraId)
        {
            ...
            float imageHeight = ArucoCameraUndistortion.CameraParameters.ImageHeights[cameraId];
            var focalLength = ArucoCameraUndistortion.RectifiedCameraMatrices[cameraId].GetCameraFocalLengths();
            var principalPoint = ArucoCameraUndistortion.RectifiedCameraMatrices[cameraId].GetCameraPrincipalPoint();
            ...
            var localPositionX = (0.5f * imageWidth - principalPoint.x) / focalLength.x * cameraBackgroundDistance;
            var localPositionY =
                -(0.5f * imageHeight - principalPoint.y) / focalLength.y *
                cameraBackgroundDistance; // a minus because OpenCV camera coordinates origin is top - left, but bottom-left in Unity
            ...
            var localScaleX = imageWidth / focalLength.x * cameraBackgroundDistance;
            var localScaleY = imageHeight / focalLength.y * cameraBackgroundDistance;

            // Place and scale the background
            Backgrounds[cameraId].transform.localPosition =
                new Vector3(localPositionX, localPositionY, cameraBackgroundDistance);
