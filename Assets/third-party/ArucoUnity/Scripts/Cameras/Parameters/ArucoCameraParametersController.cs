  ...
  public class ArucoCameraParametersController : MonoBehaviour, IHasArucoCameraParameters
    {
        ...
        [SerializeField] [Tooltip("Automatically load the camera parameters file at start.")]
        private bool autoLoadFile = true;

        [SerializeField] [Tooltip("The xml file where to load and save the camera parameters.")]
        ...
        private string cameraParametersFolderPath = "third-party/ArucoUnity/CameraParameters/";
      ...
      /// </summary>
      public bool AutoLoadFile
        {
            get { return autoLoadFile; }
            set { autoLoadFile = value; }
        }

      /// <summary>
      ///     Gets or sets the folder of the camera parameters files, relative to the
      ///     <see cref="Application.persistentDataPath" /> folder.
      /// </summary>
      public string CameraParametersFolderPath
        {
            get { return cameraParametersFolderPath; }
            set { SetCameraParametersFilePath(value, CameraParametersFilename); }
        }

      /// <summary>
      ///     Gets or sets the xml file corresponding to the camera parameters, relative to
      ///     <see cref="CameraParametersFolderPath" /> folder.
      /// </summary>
      public string CameraParametersFilename
        {
            get { return cameraParametersFilename; }
            set { SetCameraParametersFilePath(CameraParametersFolderPath, value); }
        }
      ...
      /// <summary>
      ///     Calls <see cref="SetCameraParametersFilePath" /> then <see cref="Load" /> if
      ///     <see cref="CameraParametersFilePath" /> is set.
      /// </summary>
      protected virtual void Awake()
        {
            ...
            if (AutoLoadFile)
            {
                if (CameraParametersFilePath.Length == 0)
                    throw new Exception(
                        "Cant't load the camera parameters file: CameraParametersFolderPath and CameraParametersFilename must be set.");
                Load();
            }
        }
      ...
      /// </summary>
      protected virtual void SetCameraParametersFilePath(string cameraParametersFolderPath,
            string cameraParametersFilename)
        {
            CameraParametersFilePath = "";
            if (cameraParametersFolderPath != null && cameraParametersFolderPath.Length > 0
                                                   && cameraParametersFilename != null &&
      ...
      /// <summary>
      ///     Calls <see cref="ArucoCameraParameters.SaveToXmlFile(string)" /> with <see cref="CameraParametersFolderPath" />.
      ///     Also creates the
