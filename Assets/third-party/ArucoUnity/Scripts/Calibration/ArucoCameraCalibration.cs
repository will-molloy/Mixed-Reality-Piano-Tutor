  ...
  public abstract class ArucoCameraCalibration : ArucoObjectDetector
    {
        ...
        [SerializeField] [Tooltip("The ArUco board to use for calibration.")]
        ...
        [Tooltip(
            ...
            " (if using a board).")]
        private bool refineMarkersDetection;
      ...
      /// </summary>
      public ArucoBoard CalibrationBoard
        {
            get { return calibrationBoard; }
      ...
      ///     the board layout.
      /// </summary>
      public bool RefineMarkersDetection
        {
            get { return refineMarkersDetection; }
            set { refineMarkersDetection = value; }
        }
      ...
      public virtual void DetectMarkers()
        {
            ...
            for (var cameraId = 0; cameraId < ArucoCamera.CameraNumber; cameraId++)
            {
                ...
                if (RefineMarkersDetection)
                    Aruco.RefineDetectedMarkers(ArucoCamera.Images[cameraId], CalibrationBoard.Board,
                        MarkerCorners[cameraId],
      ...
      public virtual void AddImages()
        {
            if (!IsConfigured)
                throw new Exception(
                    "Configure the calibration controller before adding the current images for calibration.");

            // Check for validity
            var markerIdsNumber = MarkerIds[0] != null ? MarkerIds[0].Size() : 0;
            for (var cameraId = 0; cameraId < ArucoCamera.CameraNumber; cameraId++)
            {
                if (MarkerIds[cameraId] == null || MarkerIds[cameraId].Size() < 1)
                    throw new Exception("No markers detected for the camera " + (cameraId + 1) + "/" +
                                        ArucoCamera.CameraNumber +
                ...
                if (markerIdsNumber != MarkerIds[cameraId].Size())
                    throw new Exception(
                        "The cameras must have detected the same number of markers to add the current images" +
                        " for the calibration.");
            }

            // Save the images and the detected corners
            var cameraImages = ArucoCamera.Images;
            for (var cameraId = 0; cameraId < ArucoCamera.CameraNumber; cameraId++)
            {
      ...
      public virtual void CalibrateAsync()
        {
            ...
            if (calibrationRunning)
                throw new Exception(
                    "A calibration is already running. Wait its completion or call CancelCalibrateAsync()" +
      ...
      public virtual void CancelCalibrateAsync()
        {
            if (!IsConfigured)
                throw new Exception(
                    "Configure the calibration controller before starting or canceling the calibration.");
            ...
            if (!calibrationRunning) throw new Exception("Start the async calibration before canceling it.");
      ...
      public virtual void Calibrate()
        {
            ...
            var charucoBoard = CalibrationBoard.Board as Aruco.CharucoBoard;
            for (var cameraId = 0; cameraId < ArucoCamera.CameraNumber; cameraId++)
            ...
            var imagePoints = new Std.VectorVectorPoint2f[ArucoCamera.CameraNumber];
            var objectPoints = new Std.VectorVectorPoint3f[ArucoCamera.CameraNumber];
            for (var cameraId = 0; cameraId < ArucoCamera.CameraNumber; cameraId++)
            {
                imagePoints[cameraId] = new Std.VectorVectorPoint2f();
                objectPoints[cameraId] = new Std.VectorVectorPoint3f();

                var imagesCount = AllMarkerCorners[cameraId].Size();
                for (uint imageId = 0; imageId < imagesCount; imageId++)
                {
                    ...
                    {
                        // Using a grid board
                        Aruco.GetBoardObjectAndImagePoints(CalibrationBoard.Board,
                            AllMarkerCorners[cameraId].At(imageId),
                    ...
                    else
                    {
                        ...
                        Aruco.InterpolateCornersCharuco(AllMarkerCorners[cameraId].At(imageId),
                            AllMarkerIds[cameraId].At(imageId),
                        ...
                        var markerCount = charucoIds.Size();
                        for (uint marker = 0; marker < markerCount; marker++)
                        {
                            var pointId = (uint) charucoIds.At(marker);
                            var objectPoint = charucoBoard.ChessboardCorners.At(pointId);
      ...
      protected virtual void InitializeCameraParameters()
        {
            ...
            {
                if (CameraParametersController.CameraParameters == null ||
                    CameraParametersController.CameraParameters.CameraMatrices == null)
                    throw new Exception(
                        "CalibrationFlags.UseIntrinsicGuess flag is set but CameraParameters is null or has no" +
            ...
            for (var cameraId = 0; cameraId < ArucoCamera.CameraNumber; cameraId++)
            {
                CameraParametersController.CameraParameters.ImageHeights[cameraId] =
                    ArucoCamera.Images[cameraId].Size.Height;
                CameraParametersController.CameraParameters.ImageWidths[cameraId] =
                    ArucoCamera.Images[cameraId].Size.Width;
            }
        }
      ...
      protected abstract void Calibrate(Std.VectorVectorPoint2f[] imagePoints,
