...
namespace Valve.VR.InteractionSystem
{
    ...
    public class ControllerHoverHighlight : MonoBehaviour
    {
        ...
        {
            if (renderModelLoaded == false) return;

            if (fireHapticsOnHightlight) hand.controller.TriggerHapticPulse(500);

            if (bodyMeshRenderer != null) bodyMeshRenderer.enabled = true;

            if (trackingHatMeshRenderer != null) trackingHatMeshRenderer.enabled = true;
        }
        ...
        public void HideHighlight()
        {
            ...
            if (fireHapticsOnHightlight) hand.controller.TriggerHapticPulse(300);

            if (bodyMeshRenderer != null) bodyMeshRenderer.enabled = false;

            if (trackingHatMeshRenderer != null) trackingHatMeshRenderer.enabled = false;
        }
    }
}