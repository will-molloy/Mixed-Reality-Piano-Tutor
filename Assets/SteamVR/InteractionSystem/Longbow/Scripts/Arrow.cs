...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Arrow : MonoBehaviour
    {
        ...
        public void ArrowReleased(float inputVelocity)
        {
            ...
            if (glintParticle != null) glintParticle.Play();

            if (gameObject.GetComponentInChildren<FireSource>().isBurning) fireReleaseSound.Play();

            // Check if arrow is shot inside or too close to an object
            var hits = Physics.SphereCastAll(transform.position, 0.01f, transform.forward, 0.80f,
        ...
        private void OnCollisionEnter(Collision collision)
        {
            if (inFlight)
            {
                ...
                var canStick = targetPhysMaterial != null && collision.collider.sharedMaterial == targetPhysMaterial &&
                               rbSpeed > 0.2f;
                ...
                if (glintParticle != null) glintParticle.Stop(true);

                // Only play hit sounds if we're moving quickly
                if (rbSpeed > 0.1f) hitGroundSound.Play();
                ...
                if (canStick) StickInTarget(collision, travelledFrames < 2);
        ...
        {
            if (scaleParentObject != null) Destroy(scaleParentObject);
        }
    }
}