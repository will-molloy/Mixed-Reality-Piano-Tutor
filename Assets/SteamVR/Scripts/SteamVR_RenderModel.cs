...
public class SteamVR_RenderModel : MonoBehaviour
{
        ...
        using (var holder = new RenderModelInterfaceHolder())
        {
            ...
            if (count > 0)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    capacity = renderModels.GetComponentRenderModelName(renderModelName, componentName.ToString(), null,
                        0);
                    if (capacity == 0)
                        continue;

                    var name = new StringBuilder((int) capacity);
                    if (renderModels.GetComponentRenderModelName(renderModelName, componentName.ToString(), name,
                            capacity) == 0)
                        continue;
    ...
    private bool LoadComponents(RenderModelInterfaceHolder holder, string renderModelName)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if (renderModels.GetComponentRenderModelName(renderModelName, componentName.ToString(),
    ...
    public void UpdateComponents(CVRRenderModels renderModels)
    {
        ...
        for (var i = 0; i < t.childCount; i++)
        {
            ...
            if (!renderModels.GetComponentState(renderModelName, name, ref controllerState, ref controllerModeState,
                ref componentState))
                continue;
